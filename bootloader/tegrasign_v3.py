...
def parse_cmdline(commandLine):
    ...
    parser.add_argument("--hsm",    help="Invoke HSM-mode call with key type specified: \
        [ |sbk|kek0|fskp_ak|fskp_ek|fskp_kdk|fskp|[index]rsa|eddsa|ecc|ecc521|xmss|algo <magic_id> <revoke> ]", nargs='*', metavar='HSM')
    ...
    parser.add_argument("--pubkeyhash", help="Specify the files to save public key, hash, mode(optional), and  RSA exponent(optional). \
        1) <public key> [hash file] when --key <private key> is used. \
        2) <public key> <hash file> <rsa|eddsa|ecdsa|xmss> when --key <private key> is not used.\
            Note in this case the public key is specific format and the key type flag is required\
        3) <public key> <hash file> <rsa> <exponent_file>", metavar='FILE', nargs='*')
...
def extract_key(p_key, keyfilename, internal):
    pk, _, _, exp = get_pkh_args(internal)
    ...
    # Handle HSM route next
        ...
        # Check to see if active index is defined and set as key id to be picked up
        ...
        if p_key.hsm.type in [KeyType.SBK, KeyType.KEK0]:
           ...
           if internal["--pubkeyhash"]:
               return is_PKC_pubkey(keyfilename, p_key, pk, mont, exp) \
                   or is_PKC_key(keyfilename, p_key, pk, mont, exp)
           return 1
        ...
        elif p_key.hsm.type == KeyType.ECC521:
            p_key.mode = NvTegraSign_ECC521
            p_key.keysize = NV_ECC521_SIG_STRUCT_SIZE
            if internal["--pubkeyhash"]:
                return is_ECC_key(keyfilename, p_key, pk)
            return 1

        elif p_key.hsm.type == KeyType.XMSS:
           p_key.mode = NvTegraSign_XMSS
           p_key.keysize = XMSS_SIG_SIZE
           if internal["--pubkeyhash"]:
               return is_xmss_key(keyfilename, p_key, pk)
           return 1

        elif p_key.hsm.type == NvTegraSign_LIST:
            return 1
        ...
        if is_key_list(keyfilename, p_key, internal["--verbose"]):
           p_key.mode = NvTegraSign_LIST
           p_key.hsm.type = NvTegraSign_LIST
           p_key.filename = keyfilename
           return 1

        if is_PKC_pubkey(keyfilename, p_key, pk, mont, exp) is True \
            or is_PKC_key(keyfilename, p_key, pk, mont, exp) is True:
           p_key.mode = NvTegraSign_PKC
           p_key.filename = keyfilename
           return 1
...
def extract_pubkey(p_key, internal):
    ...
    pk, pkh, mode, exp = get_pkh_args(internal)
    p_key.mode = mode
...
def save_public_key(p_key, internal):
    pk, _, _, _ = get_pkh_args(internal)
    if check_file(pk):
        if p_key.mode == NvTegraSign_PKC:
...
def save_public_key_hash(p_key, internal):
    ...
    if pkh == None:
        return 1
    if p_key.pcp.deviceid.is_t264():
       # this prints the hash values
        p_key.pk_file = pk
        p_key.pkh_file = pkh
        return save_public_key_hash_t264(p_key, internal)
    elif p_key.kdf.deviceid.is_t264():
        # this is for t264 bch pcp
        PCP_SIZE = 388
        start = 4
    else:
        PCP_SIZE = 384
    ...
    with open(pcp_file, 'wb') as fw:
        buf = bytearray(PCP_SIZE)
        if p_key.kdf.deviceid.is_t264():
            algo_list = { KeyType.PKC:1, KeyType.ECC:2, KeyType.ECC521:3, KeyType.ED25519:4, KeyType.XMSS:5}
            buf[0] = algo_list[p_key.hsm.type]
        buf[start:start+keysize] = pub_bytes[:]
        fw.write(buf)
    if check_file(pcp_file) == False:
        return 0

    if exp != None:
        get_prop_value = import_function('get_prop_value')
        oem_val = get_prop_value(p_key, 'OEM_CUSTOM_VALUE')
        with open(exp, 'rb') as fe, open(pk, 'rb') as fp:
            pcp_bytes = fp.read()
            exp_bytes = fe.read()
            # exit after print pcp and exp
            print_ist_pcp(pcp_bytes, exp_bytes, oem_val)
            return 1
    ...
    with open(pkh, 'rb') as f:
        ...
        if p_key.kdf.deviceid.is_t264():
            # update pcp file to account for the active scheme
            os.rename(pcp_file, pk)
        else:
            os.remove(pcp_file)
        return 1
    return 0

def save_public_key_hash_t264(p_key, internal):
    command = exec_file(TegraOpenssl)
    command.extend(['--chip'])
    command.extend(p_key.pcp.deviceid.chipid_all())
    command.extend(['--pubkeyhash', p_key.pcp.signed_xml_file])
    if internal['--verbose']:
        command.extend(['--verbose'])

    ret_str = run_command(command)

    if p_key.pcp.pcp_file != None and os.path.exists(p_key.pcp.pcp_file):
        shutil.copyfile(p_key.pcp.pcp_file, p_key.pk_file)
        info_print('%s is produced from copying %s' %(p_key.pk_file, p_key.pcp.pcp_file))
    else:
        info_print('Can not produce %s' %(p_key.pk_file))

    if p_key.pcp.pcps_hash_file != None and os.path.exists(p_key.pcp.pcps_hash_file):
        shutil.copyfile(p_key.pcp.pcps_hash_file, p_key.pkh_file)
        info_print('%s is produced from copying %s' %(p_key.pkh_file, p_key.pcp.pcps_hash_file))
    else:
        info_print('Can not produce %s' %(p_key.pcp.pcps_hash_file))

    with open(p_key.pcp.pcps_hash_file, 'rb') as f:
        hash_bytes = f.read()
        print_pcp(hash_bytes)
        if internal['--verbose']:
            print_pcp_entries(hash_bytes, p_key.mode)
        return 1
...
def print_args(internal):
    ...
    try:
        ...
        if internal["--pubkeyhash"]:
            if isinstance(internal["--pubkeyhash"], list):
                argstr += ' --pubkeyhash ' + ' '.join(internal["--pubkeyhash"])
            else:
...
def tegrasign(args_file, args_getmode, args_getmont, args_key, args_length, args_list, args_offset, args_pubkeyhash, args_sha, args_enc, args_verbose=False, args_iv=0, args_aad=0, args_tag=0, args_sign=None, args_verify=0, args_kdf=None, args_hsm=None, args_ran=None, args_block='0', args_softhsm=False):
    ...
    try:
        ...
        # filter out hsm sign routine since this flag is used for encryption
        if is_kdf_file and  len(internal["--kdf"]) == 1 and internal["--pubkeyhash"] and \
            len(internal["--pubkeyhash"]) >= 2:
            is_kdf_file = False
        ...
        if internal["--key"] or internal["--hsm"] or is_kdf_file:
            ...
            if is_key_list:
                ...
                if internal["--hsm"]:
                    ...
                    if is_hsm() and is_softhsm():
                        info_print("[HSM] HSM key label: %s" % p_keylist[0].hsm.l4t_key_label)
                        from tegrasign_v3_internal import is_key_list
                        is_key_list(internal["--key"], p_keylist[0], internal['--verbose'])
                    else:
                        create_key_list(p_keylist[0], internal)

                if internal["--kdf"]:
                    p_keylist[0].kdf.parse(p_keylist[0], internal)
                    if not is_kdf_file:
                        # reset so extract key can update mode
                        p_keylist[0].mode = "Unknown"
            ...
            if internal["--pubkeyhash"]:
                ...
                if type(internal["--pubkeyhash"]) == list and len(internal["--pubkeyhash"]) > 4:
                    info_print('%s flag is not needed since --key is used' %(internal["--pubkeyhash"][4]))
                    print_help()
                else:
