...
def run_command(cmd, enable_print=True, ignore=False):
    log = ''
...
def tegraflash_t19x_encrypt_and_sign(cfg_file):
    ...
    call_tegrasign(None, None, None, key_val, None, list_val, None, tegraflash_pubkeyhash_for_sign(key_val, pkh_val), None, None)
    ...
    if int(values['--chip'], 0) == 0x19:
        if mode == 'oem-rsa':
            if os.path.isfile(tegrasign_values['--getmontgomeryvalues']):
                mont_val = tegrasign_values['--getmontgomeryvalues']
    call_tegrasign(None, None, mont_val, key_val, None, filename + '_list.xml', None, tegraflash_pubkeyhash_for_sign(key_val, pkh_val), None, None)
...
def tegraflas_oem_sign_file(in_file, magic_id):
    ...
    if int(values['--chip'], 0) in [0x19, 0x23] :
        if mode == 'oem-rsa':
            if os.path.isfile(tegrasign_values['--getmontgomeryvalues']):
                mont_val = tegrasign_values['--getmontgomeryvalues']
    call_tegrasign(None, None, mont_val, key_val, None, list_val, None, tegraflash_pubkeyhash_for_sign(key_val, pkh_val), None, None)
...
def tegraflash_signing_key_is_placeholder(key_val):
    """True when --key was left at default SBK placeholder (['None'] / 'None'), not a PKC file."""
    if key_val is None:
        return True
    if key_val == 'None':
        return True
    if isinstance(key_val, list):
        if len(key_val) == 0:
            return True
        return all((k is None or k == 'None') for k in key_val)
    return False

def tegraflash_pubkeyhash_for_sign(key_val, pkh_val):
    """
    Pass pubkeyhash to tegrasign only when a real asymmetric key path is used.
    With default --key ['None'], tegrasign does not write pub_key.key to avoid passing
    --pubkeyhash then triggers save_public_key / check_file warnings.
    """
    if pkh_val is None:
        return None
    if tegraflash_signing_key_is_placeholder(key_val):
        return None
    return pkh_val
...
def tegraflash_generate_br_bct(coldboot_bct):
    ...
    if values['--encrypt_key'] is not None:
       ...
       if int(values['--chip'], 0) == 0x19:
           ...
           pkh_val = tegrasign_values['--pubkeyhash']
       call_tegrasign(file_val, None, None, key_val, None, list_val, offset_val, tegraflash_pubkeyhash_for_sign(key_val, pkh_val), None, None)
...
def set_partition_filename(match_str, filename, field='name', is_ignore=True):
    with open(values['--cfg'], 'r+') as file:
        ...
        for node in root.iter('partition'):
            if(node.get(field) == match_str):
                ...
                if file_node.text == None and is_ignore == True:
                    info_print("skip the partition without specific file")
                    continue
                info_print('Change ' + str(file_node.text) + ' to ' + filename)
                file_node.text = filename

        xml_tree.write(values['--cfg'])
    return

def remove_partition_filename(match_str, field='name'):
    with open(values['--cfg'], 'r+') as file:
        xml_tree = ElementTree.parse(file)
        root = xml_tree.getroot()
        for node in root.iter('partition'):
            if(node.get(field) == match_str):
                ...
                if file_node != None:
                    ...
                    info_print("Remove filename node for %s partition" %(match_str))
                    break

        xml_tree.write(values['--cfg'])
    return

def get_partition_filename(match_str, field='name', pt=None):
    if pt == None:
        pt = values['--cfg']
    with open(pt, 'r') as file:
        xml_tree = ElementTree.parse(file)
...
def is_partition_present(match_str, field='name', pt=None):
    if pt == None:
        pt = values['--cfg']
    with open(pt, 'r') as file:
        xml_tree = ElementTree.parse(file)
...
def tegraflash_add_odm_data_to_dtb (odmdata_str, dtb_file):
    ...
    try:
        ...
        for prop in odm_list:
            if prop.endswith('-del'):
                prop = prop.replace('-del','')
                try:
                    chosen_node.remove(cfg)
                except:
                    pass
            else:
...
def tegraflash_generate_rcm_message(is_pdf = False):
    ...
    if int(values['--chip'], 0) in [0x19, 0x23]:
        ...
        if mode == "pkc":
            mont_val = tegrasign_values['--getmontgomeryvalues']
        call_tegrasign(filename, None, mont_val, key_val, length_val, None, offset_val, tegraflash_pubkeyhash_for_sign(key_val, pkh_val), None, None)
    ...
    if not values['--tegraflash_v2'] and int(values['--chip_major'], 0) >= 2 and values['--encrypt_key'] is not None:
        ...
        call_tegrasign(None, None, None, values['--encrypt_key'][-1], None, tegrarcm_values['--list'],  None, tegraflash_pubkeyhash_for_sign(values['--encrypt_key'][-1], tegrasign_values['--pubkeyhash']), None, None)
