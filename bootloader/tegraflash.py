...
try:
    from tegraflash_impl_t264 import (TFlashT264)
except ImportError:
    print("WARNING: failed to import T264 module")
...
exports = {
            ...
            "--mb1_bin":None, "--psc_bl1_bin":None, "--no_pva":"0",
            ...
            "--iv": "RANDOM", "--no_flash": False, "--key_list": None, "--compress": None, "--enable_mods": None,
            "--hsm": False,
            "--bct_flags_file": None, "--overlay_bct_flags_file": None,
            "--rawkerneldtb": None
          }
...
def usage():
    ...
    '                    [--mb1_bin] [--psc_bl1_bin], [--dry_run], [--no_pva <0|1>]',
    '                    [--coldboot_pt_layout], [--rcmboot_pt_layout], [--coldboot_bct_cfg], [--rcmboot_bct_cfg], [--duk]',
    '                    [--dce_base_dtb], [--dce_overlay_dtb], [--disable_random_iv], [--no_flash], [--key_list]',
    '                    [--compress <image_type>], [--hsm], [--bct_flags_file <cflags yaml>]',
    '                    [--overlay_bct_flags_file] <override yaml file>'
    '                    [--rawkerneldtb <file>]'
    ...
    '   --key_list      : Key list for signing required files (only for specific chips)',
    '   --encrypt_key   : Key for encrypting required files',
    '   --enable_user_kdk : Enable user defined KDK',
    '   --enable_mods   : Enable mods to set disable_preprod_firewalls in mb1bct',
    ...
    '   --no_pva        : Option to skip flashing PVA partition'
    ...
    '   --bct_flags_file: File that stores the L4T CFLAGS',
    '   --overlay_bct_flags_file: Override yaml file that stores the common L4T CFLAGS',
    '   --rawkerneldtb  : Raw kernel DTB that is not signed or encrypted and is not applied by any overlay'
    '   '
    ]))
...
class tegraflashcmds(cmd.Cmd):
    ...
    def do_sign(self, params):
        ...
        try:
            if len(args) == 0:
                ...
                # Only route the following condition for t234 or t264, old chips remain the same path
                if int(exports['--chip'], 0) == 0x23 or int(exports['--chip'], 0) == 0x26:
                    if exports['--encrypt_key'] is None:
    ...
    def do_softfuses(self, params):
        ...
        if len(args) <= 2:
            ...
            try:
                self.chip_inst.tegraflash_softfuses(exports, args)
            except tegraflash_exception as e:
                print('Error: '+ e.value)
                tegraflash_err(1)
        else:
    ...
    def help_softfuses(self):
        print('\n'.join([
        ' ',
        '----------------------------------------------------------------------',
        '  T264 Usage: softfuses <filename.xml> ',
        '----------------------------------------------------------------------',
    ...
    def do_build(self, params):
       ...
       try:
           self.chip_inst.tegraflash_build(exports, args)
...
if __name__ == '__main__':
    options = ["skipuid", "bct=", "bct_cold_boot=", "cfg=", "bl=", "hostbin=", "cmd=", "key=", "encrypt_key=","instance=",
               ...
               "compress=", "enable_mods", "hsm", "bct_flags_file=", "overlay_bct_flags_file=", "rawkerneldtb="]
    ...
    if '--enable_mods' in sys.argv[1:]:
        exports['--enable_mods'] = True
