...
class TFlashT264_Base(object):
    ...
    def tegraflash_generate_signing_list(self, pt_file=None):
        ...
        if values['--minratchet_config'] is not None and not values['--external_device']:
    ...
    def tegraflash_generate_ratchet_blob(self):
        if not os.path.exists(self.tegrahost_values['--ratchet_blob']):
            ...
            self.tegraflash_preprocess_configs(configs_dict, ['minratchet'])
    ...
    def tegraflash_generate_br_bct(self, configs_dict, bct_flags=[], magicid=None):
        ...
        br_bct_configs_list = ['dev_param', 'sdram']
        bct_flags = bct_flags[:]
        bct_flags.append('MEM_BCT')

        self.tegraflash_preprocess_configs(configs_dict, br_bct_configs_list, bct_flags)
        ...
        command.extend(['--dev_param', configs_dict['dev_param']])
        if 'sdram' in configs_dict:
            command.extend(['--sdram', configs_dict['sdram']])
    ...
    def tegraflash_generate_hpse_sb_pkg(self, bct_cfg=None, bct_flags=['MEM_BCT']):
        ...
        for pkg in pkg_types:
            ...
            if self.tegraflash_can_build_hpse_or_sb(configs_dict) == False:
               ...
               continue
            bct_file_name = self.tegraflash_generate_br_bct(configs_dict, bct_flags=bct_flags, magicid=pkg_types[pkg][0])
    ...
    def tegraflash_generate_blob(self, sign_images, blob_filename):
        ...
        for partition in rcm_dev.iter('partition'):
            ...
            if (file_node == None or img_type == 'bootloader'):
                continue
    ...
    def tegraflash_generate_mb1_bct(self, configs_dict, bct_flags=[]):
        ...
        if (values["--mb1_bct"] is not None and values["--mb1_bct"].endswith(".bct")):
            ...
            mb1_bct_preprocess_list = ['misc', 'wb0sdram', 'pinmux', 'pmc', 'pmic', 'br_cmd', 'prod', 'gpioint',
                                   'uphy', 'device', 'deviceprod', 'minratchet']

            self.tegraflash_preprocess_configs(configs_dict, mb1_bct_preprocess_list, bct_flags)
            ...
            if ('misc'in configs_dict):
                command.extend(['--misc', configs_dict['misc']])
            if 'wb0sdram' in configs_dict:
                command.extend(['--wb0sdram', configs_dict['wb0sdram']])
            if 'pinmux' in configs_dict:
                command.extend(['--pinmux', configs_dict['pinmux']])
            if 'pmc' in configs_dict:
                command.extend(['--pmc', configs_dict['pmc']])
            if 'pmic' in configs_dict:
                command.extend(['--pmic', configs_dict['pmic']])
            if 'br_cmd' in configs_dict:
                command.extend(['--brcommand', configs_dict['br_cmd']])
            if 'prod' in configs_dict:
                command.extend(['--prod', configs_dict['prod']])
            if 'gpioint' in configs_dict:
                command.extend(['--gpioint', configs_dict['gpioint']])
            if 'uphy' in configs_dict:
                command.extend(['--uphy', configs_dict['uphy']])
            if 'device' in configs_dict:
                command.extend(['--device', configs_dict['device']])
            if 'deviceprod' in configs_dict:
                command.extend(['--deviceprod', configs_dict['deviceprod']])
            if values['--fb'] is not None:
                command.extend(['--fb', values['--fb']])
            if values['--minratchet_config'] is not None:
                command.extend(
                    ['--minratchet', configs_dict['minratchet']])
                command.extend(
            ...
            if self.tegraparser_values['--pt'] is not None:
                ...
                info_print('Updating mb1-bct with storage device information')
                command = self.exec_file('tegrabct')
                command.extend(['--chip', values['--chip'], values['--chip_major']])
                command.extend(['--mb1bct', mb1_bct_file_name])
    ...
    def generate_mem_bcts(self, configs_dict, bct_flags=[]):
        mem_bct_preprocess_list = ['wb0sdram', 'sdram']

        bct_flags = bct_flags[:]
        bct_flags.append('MEM_BCT')
        self.tegraflash_preprocess_configs(configs_dict, mem_bct_preprocess_list, bct_flags)
        ...
        mem_bcts = [
            filename + "_1.bct", filename + "_2.bct", filename + "_3.bct", filename + "_4.bct",
            filename + "_5.bct", filename + "_6.bct", filename + "_7.bct", filename + "_8.bct",
        ]
        command.extend(['--membct',
    ...
    def generate_bpmp_mem_bins(self, configs_dict, bct_flags=[]):
        bpmp_mem_cfg_preprocess_list = ['bpmp_mem_cfg']
        bct_flags = bct_flags[:]
        bct_flags.append('BPMP_MEM_CFG')

        self.tegraflash_preprocess_configs(configs_dict, bpmp_mem_cfg_preprocess_list, bct_flags)
        ...
        command = self.exec_file('tegrabct')
        command.extend(['--chip', values['--chip'], values['--chip_major']])
        command.extend(['--bpmp_mem_cfg', configs_dict['bpmp_mem_cfg']])

        filename = os.path.splitext(configs_dict['bpmp_mem_cfg'])[0]
        bpmp_mem_bins = [filename + "_1.bin", filename + "_2.bin",
                    filename + "_3.bin", filename + "_4.bin",
                    filename + "_5.bin", filename + "_6.bin",
                    filename + "_7.bin", filename + "_8.bin" ]
        command.extend(['--bpmp_mem_bin', bpmp_mem_bins[0], bpmp_mem_bins[1],
                        bpmp_mem_bins[2], bpmp_mem_bins[3], bpmp_mem_bins[4],
                        bpmp_mem_bins[5], bpmp_mem_bins[6], bpmp_mem_bins[7]])
        run_command(command)

        return bpmp_mem_bins
    ...
    def tegraflash_dumpeeprom(self, args, params):
        ...
        try:
            ...
            info_print("Cannot get the eeprom")
    ...
    def tegraflash_generate_mb2_bct(self, configs_dict, bct_flags=[]):
        ...
        self.tegraflash_preprocess_configs(configs_dict, mb2_bct_preprocess_list, bct_flags)
        ...
        command.extend(['--mb2bct', mb2_bct_file_name])
        command.extend(['--mb2bctcfg', configs_dict['mb2bctcfg']])

        if 'scr' in configs_dict:
            command.extend(['--scr', configs_dict['scr']])
        ...
        command = self.exec_file('tegrabct')
        command.extend(['--chip', values['--chip'], values['--chip_major']])
        command.extend(['--mb2bct', values['--mb2_bct']])
        command.extend(['--updatestorageinfo', self.tegraparser_values['--pt']])
        run_command(command)
