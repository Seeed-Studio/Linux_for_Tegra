...
/**
 * @brief Defines a structure to store/hold carveout information.
 *        It tracks the base and size of the Carveout.
 */

struct tegrabl_carveout_info {
	/** Base address of the carveout region */
	uint64_t base;
	/** Size of the carveout region*/
	uint64_t size;
	union {
		struct {
			/** Is the carveout ECC protected*/
			uint64_t ecc_protected:1;
			/** Reserved bits for future use */
			uint64_t reserved:63;
		};
		/** Carveout allocation flags*/
		uint64_t flags;
	};
};
...
/**
 * @addtogroup CARVEOUT_IDS
 * @brief Define the values for tegrabl_carveout_t
 *
 * @{
 */
...
#define CARVEOUT_TEMP_FSI_LOAD						21U
