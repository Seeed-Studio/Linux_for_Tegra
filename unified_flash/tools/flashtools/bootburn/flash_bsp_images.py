...
#            ex: -b <board-name> -h
#       '--find_board_name' is supported only t23x
#FIXME
#     Implement SOC data reader from Aurix info and call
#      corresponding SOC stack for -h' option and
#      '--find_board_name' option
...
if __name__ == "__main__":
    ...
    if socgrpSelect.isSocGrpFound():
        flash_bsp_module = importlib.import_module(f"{socgrpSelect.soc_scripts_dir}.flash_bsp_images")
        flash_bsp_func = getattr(flash_bsp_module, "flash_bsp")
        result = flash_bsp_func(sys.argv)
        sys.exit(result)
    else:
