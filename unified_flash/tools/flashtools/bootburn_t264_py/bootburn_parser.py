...
class commandline_parser(object):
    ...
    input_options = {
        ...
        "-c": (0, "Flash SCE with Safety firmware.targetConfig.   Flash APE with camera firmware. (Currently not supported)", False),
        ...
        "-f": (0, "FA(Failure Analysis) mode Support.", False),
        "-g": (1, "Generate binaries at mentioned path only and do not flash.", False),
        "-h": (0, "help message", False),  # BoardName
        "-i": (-1, "File path to Platform Configuration file. For hypervisor configurations - if this is not provided, then the tool expects Platform Configuration file to be present in hypervisor output directory", False),
        "-j": (0, "Switch between bash and python scripts", False),
        ...
        "-L": (0, "Enable Low Power modes.", False),
        ...
        "--pytest": (0, "Turn off time consuming features for pytest runs", False),
        ...
        "--encryption_key": (1, "Provide common encryption key file", False),
        "--encrypt": (0, "Global switch to enable encryption", False),
        "--pl_encryption_key": (1, "Provide OS encryption key file", False),
        ...
        "--no_du_pvit":(0, "DU PVIT entry not to be generated", False),
        "--l4t_boot_chain_select":(1, "Specify the boot chain (ex. \"A\" or \"B\" ) for L4T", "A"),
        }

    safetyAllowedOptions = ['b', 'g', 'h', 'l', 'p', 'r', 't', 'u', 'x', 'z', 'A', 'B', 'D', 'E',
                            'I', 'P', 'R', 'board_config', 'clean', 'encryption_key', 'encrypt', 'safety',
                            'user', 'chain', 'asymmetric', 'customer_data', 'pl_encryption_key',
                            'hsm', 'init_persistent_partitions', 'devicetype', 'merge_chains',
                            'fskp_bct_path', 'logs', 'find_board_name', "target_pvit",
                            "no_firmware_update", "no_du_pvit", "l4t_boot_chain_select"]
    ...
    def initializeTargetConfig(self, targetConfig, genericOptions=None, parsePlatform=True):
        ...
        try:
            if(self.parsed_commandline["h"] == True):
                ...
                return
            elif(self.parsed_commandline["find_board_name"] and (len(targetConfig.s_baseBoardName) == 0)):
    ...
    def setTargetConfigDataCmdLine(self, targetConfig, commandLine):
        ...
        for option in commandLine:
            ...
            elif(option == "L"  and argument == True):
                targetConfig.n_LoadSPEFW = argument
    ...
    def ValidateParameters(self, targetConfig):
        ...
        n_SafetyProfile = targetConfig.parsed_commandline["c"]
        ...
        if(n_SafetyProfile and targetConfig.n_EngSample):
            print("\nINVALID OPTION\n")
            print("-c not supported in bootburn")
            AbnormalTermination("s_ERROR_OPTION_VALUE_INVALID - No secure flash on internal board", nverror.NvError_NotSupported)
