...
class bootburn_thor(bootburn_lib):
    ...
    def CopyFlashingDtbImg(self, p_TempDumpPath):
        ...
        if (l_LinuxDtbImgFound):
           # Check if flashing kernel is using USB 3.0 or 2.0, change the dtb path accordingly
           if self.targetConfig.s_FlashingUsbVersion == "3":
                dtbSrcPath = os.path.join(self.targetConfig.p_FlashingDtbPath, self.targetConfig.boardDefaultPaths["f_FlashingDtbImg"])
           else:
                dtbSrcPath = os.path.join(self.targetConfig.p_FlashingDtbPath, "usb2", self.targetConfig.boardDefaultPaths["f_FlashingDtbImg"])

           dtbDestPath = os.path.join(p_TempDumpPath, self.targetConfig.s_LinuxDtbImgName)
           self.shellUtils.Copy(dtbSrcPath, dtbDestPath)
        else:
    ...
    def GenerateHpseSbPkg(self, l_Operation, p_TempDumpPath, chain):
        ...
        if (self.targetConfig.parsed_commandline["find_board_name"] != None):
            pkgDict['hpse_t264'][3] = [self.targetConfig.p_HpseFindBoardPctBin, "f_HpsePctBin", "f_HpsePctBinAbs"]
            pkgDict['sb_t264'][3] = [self.targetConfig.p_HpseFindBoardPctBin, "f_SbPctBin", "f_SbPctBinAbs"]
    ...
    def getMb1NameStem(self):
        # First finds the mb1 file name then truncate for the stem
        # return mb1_t264 by default
        # Execpted returning choices: mb1_safe_t268, mb1_t268, mb1_t264

        mb1_list = glob.glob("*mb1*_t26*.*")
        if (len(mb1_list)) > 0:
            # Pick the first one matched
            m = re.search(r'[123ABC_]*(mb1[\w_]+t[\d]+)', mb1_list[0])
            if m:
                return m.group(1)
        return "mb1_t264"
    ...
    def BootMb2Applet(self, l_Operation, binaryLocation):
        ...
        mb1_name = self.updateImageName(self.getMb1NameStem(), True)
        ...
        if (l_Operation == "rcm-boot"):
            if (self.targetConfig.parsed_commandline["multi_die"]):
                mb1bct = self.shellUtils.find('../../rcm-flash', "bct_MB1_")[0]
            else:
                mb1bct = self.shellUtils.find('../rcm-flash', "bct_MB1_")[0]
        else:
    ...
    def BootRCM(self, l_Operation, binaryLocation):
        ...
        mb1_name = bootburnThor.updateImageName(self.getMb1NameStem(), True)
