...
def create_bsp_active(bootburnLib, targetConfig, getSkuFromTarget=False):
    try:
        ...
        if not(targetConfig.parsed_commandline["l4t"] or targetConfig.parsed_commandline["vdk"]):
        ...
        if(targetConfig.parsed_commandline["toolsonly"]):
            ...
            bootburnLib.CheckIfUserSignsImages()
...
def create_bsp(commandLineArguments):
    try:
        ...
        GenericOptions = ['b', 'd', 'g', 'h', 'k', 'l', 'm', 'p', 'q', 'r',
                        's', 'u', 'B', 'C', 'D', 'E', 'M', 'R', 'T', 'X', 'Y',
                        'asymmetric', 'board_config', 'chain', 'customer_data', 'disable-uart',
                        'encrypt', 'fpga', 'fskp_bct_path', 'hsm',
                        'init_persistent_partitions', 'l4t', 'logs', 'merge_chains', 'no_du_pvit',
                        'oot_kernel','safety', 'target_pvit', 'toolsonly', 'user', 'vdk']
