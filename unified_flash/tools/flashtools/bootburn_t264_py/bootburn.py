...
def bootburn(commandLineArguments):
    ...
    debug = False
    GenericOptions = [
        'b', 'c', 'd', 'h', 'k', 'l', 'm', 'o', 'p', 'q', 's',
        't', 'u', 'x', 'y', 'B', 'C', 'D', 'E', 'I', 'L', 'M', 'P', 'R',
        'T', 'O', 'U', 'V', 'X', 'Y',
        'adbSerial', 'board_config', 'chain', 'clean', 'customer_data', 'devicetype',
        'disable-uart', 'encrypt', 'filesystem', 'find_board_name',
        'hsm', 'init_persistent_partitions', 'logs', 'no_du_pvit', 'no_firmware_update',
        'platform', 'safety', 'user']
    ...
    try:
        ...
        if(len(commandLineArguments) <= 1):
            ...
            return nverror.NvError_InvalidArgument
        ...
        if(exitcode == 0):
            if(targetConfig.s_McuPort != None) and (not (targetConfig.parsed_commandline["R"] )):
