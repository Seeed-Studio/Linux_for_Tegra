...
class bootburn_lib(object):
    ...
    # Check if user wants to sign the images
    def CheckIfUserSignsImages(self):
       if (len(self.targetConfig.HSMStr) != 0) or self.targetConfig.f_PkcKeyFilePath != None:
           self.targetConfig.b_UserSignsImages = True
    ...
    def CheckFuseForEncryption(self):
        ...
        if "create_bsp" in caller:
            ...
            if nibble[0] == '1':
                ...
                if "bootburn" in caller:
                    if encKeyProvided == True:
                        self.vlog("User provided an encryption key but target expects unencrypted images!\n" +
    ...
    def CopyTegraSign(self, l_path):
        ...
        if self.targetConfig.IsNvKeyFilesNeeded():
    ...
    def CopyFirmwareFiles(self, l_p_OutDirPath):
        ...
        #enable the following after GVS test fails are resolved
    ...
    def SkuBlobGen(self, l_BRBCT):
        ...
        updateSignedCustomerData = self.IsUpdateSignedCustomerData()
        ...
        if (((self.targetConfig.f_SignedCustomerData != None) or (self.targetConfig.signedCustomerData != {})) and
            ...
            if self.targetConfig.b_boardFusedForSigning or self.targetConfig.b_UserSignsImages:
...
#     #Parses the config file for the uid in bct partition
    ...
    def GenerateImage(self, configFileDataList, l_Image, p_TempDumpPath):
        ...
        chipID = str(self.targetConfig.GetChipID())
        chipIDMinor = str(self.targetConfig.getChipIDMinor(chipID))
        ...
        for chain in chainlist:
            ...
            if not chipIDMinor:
                ...
                l_NvImagegenCmd += " --chip " + chipID + " --chip-minor " + chipIDMinor + " --chipver A" + str(self.targetConfig.n_ChipVersion).zfill(2)
            ...
            # Get the defines list for this operation
            bctDefines = definesOb.getDefines(l_Image, self.targetConfig, partitionList, chain)

            # Now check if the define is in the list
    ...
    def generateSHA512andsignImage(self, imageToSign, imgMagicId):
        ...
        shutil.copy(imageToSign, imageToResign)
        with open(imageToResign, 'wb') as ofd:
            ofd.write(dataBuf)
        ...
        mb1bct_name = self.shellUtils.find('.', mb1bct_name)[0]
        l_Mb1BctCmd = ("dd if=%s skip=16 of=bct_MB1.bct" %(mb1bct_name))
        result = self.shellUtils.executeShellCommand(l_Mb1BctCmd)
        if (result != nverror.NvError_Success):
            AbnormalTermination("creation of bct_MB1.bct failed", nverror.NvError_BadParameter)

        l_NvImagegenCmd += " --mb1bct bct_MB1.bct "

        result = self.shellUtils.executeShellCommand(l_NvImagegenCmd)
        if (result != nverror.NvError_Success):
            AbnormalTermination("s_ERROR_TOOL_NVIMAGEGEN", nverror.NvError_NvImagegen)
        if os.path.isfile(imageToResign):
            os.remove(imageToResign)
            os.remove("bct_MB1.bct")
    ...
    def _find_partition_in_pvit_buffer(self, bytepartition, data_buf, offset, max_records, record_size, buffer_type, source_name):
        """
        Improved partition search function for PVIT buffers with better error handling and validation.

        Args:
            bytepartition: byte array of partition name to search for
            data_buf: buffer to search in
            offset: starting offset in the buffer
            max_records: maximum number of records to check
            record_size: size of each PVIT record
            buffer_type: string describing buffer type for logging ('source' or 'target')
            source_name: name of the source file for logging

        Returns:
            tuple: (entry_number, buffer_slice) if found, (None, None) if not found
        """
        entry = 1
        while entry <= max_records:
            start_pos = offset + (entry - 1) * record_size
            end_pos = offset + entry * record_size

            # Bounds check
            if end_pos > len(data_buf):
                self.vlog(f"Reached end of {buffer_type} buffer at entry {entry}")
                break

            buf_name_slice = data_buf[start_pos+2:start_pos+26]
            buf_slice = data_buf[start_pos:end_pos]

            # More robust partition matching - check for exact partition name boundaries
            if self._is_partition_match(bytepartition, buf_name_slice):
                self.vlog(f"Partition {bytepartition.decode('utf-8')} found in {buffer_type} ({source_name}) at entry {entry}")

                return entry, buf_slice

            entry += 1

        self.vlog(f"Partition {bytepartition.decode('utf-8')} not found in {buffer_type} after checking {entry-1} entries")
        return None, None

    def _is_partition_match(self, bytepartition, buf_slice):
        """
        Enhanced partition matching with better validation.

        Args:
            bytepartition: byte array of partition name to search for
            buf_slice: buffer slice to search in

        Returns:
            bool: True if partition is found with proper boundaries
        """
        if not bytepartition or not buf_slice:
            return False


        # Find the partition name in the buffer
        if bytepartition in buf_slice:
            # Additional validation: check that it's not a partial match
            len_buf_slice = 0
            start_pos = 0
            end_pos = len(buf_slice)
            while (start_pos < end_pos):
                next_char = buf_slice[start_pos]
                if next_char != 0 and next_char != ord(' ') and next_char != ord('\x00'):
                    len_buf_slice += 1
                else:
                    break
                start_pos += 1
            if len_buf_slice == len(bytepartition):
                return True
            else:
                return False
    ...
    def updateTargetPvitInPartial(self,chain, flashImgPath, adbObj=None):
        ...
        for partName in self.targetConfig.d_PvitChainNPartitions[chain]["partList"]:
            ...
            bytepartition = bytearray(partition, 'utf-8')

            if ("pvit" in partition):
                continue

            # Find partition in source and destination buffers using improved comparison
            src_entry, src_buf = self._find_partition_in_pvit_buffer(
                bytepartition, src_data_buf, 0, maxNumPvitRecords, pvitRecordSize,
                "source", self.targetConfig.d_PvitChainNPartitions[chain]["pvitImg"]
            )
            if src_entry is None:
                self.vlog(f"Partition {partition} not found in generated PVIT partition image")
                self.vlog(f"src_data_buf :: {src_data_buf}")
                AbnormalTermination(f"Selected partition {partition} not found in partial Flashing PVIT", nverror.NvError_ImageMismatch)

            dst_entry, dst_buf = self._find_partition_in_pvit_buffer(
                bytepartition, dst_data_buf, bchSize, maxNumPvitRecords, pvitRecordSize,
                "target", pvitImg_UsrRTarget
            )
            if dst_entry is None:
                self.vlog(f"Partition {partition} not found in target PVIT partition")
                self.vlog("No bct partition found in target PVIT, skip the bct update")
                self.vlog(f"dst_buf :: {dst_buf if 'dst_buf' in locals() else 'Not available'}")
                AbnormalTermination(f"Selected partition {partition} not found in Target PVIT", nverror.NvError_ImageMismatch)

            # Store the found entries for later use
            srcPvitEntry = src_entry
            tgtPvitEntry = dst_entry
    ...
    def UpdatePVITFullFlashing(self, adbObj, flashPath):
        ...
        #    the PVIT get signed.
        self.log ("\n ***   In UpdatePVITFullFlashing   ***\n")
        if (( "flash_bsp_images" in sys.argv[0]) and (self.targetConfig.b_boardFusedForSigning or self.targetConfig.n_SecurebootFlash)):
        ...
        for bctPartEntry in bctEntries:
            self.log(f"bctPartEntry ::  {bctPartEntry}")
        if len(bctEntries.keys()) == 0:
            self.log(f"no BR BCT partition found in FileToFlash.txt")
            return
        ...
        for bctWithChain in bctEntries:
            ...
            if (NVDAMagicWord in str(data_buf[:partMagicSize])) and (pvitMagicWord in str(data_buf[partMagicOffset:partMagicOffset+partMagicSize])):
                ...
                self.log( pvitImg_generated + " is with BCH")
            else:
                data_buf_no_bch = data_buf
                no_bch = True
                self.log( pvitImg_generated + " is without  BCH")
            brBctSha = self.getBrBctCryptoSha(bctWithChain, flashPath, adbObj)
            self.log(f"BR BCT signed customer data updated for partition bct ...updating PVIT with new SHA {brBctSha.hex()}")
            # Copy the sha of BCT into PVIT
            partNotFound = True
            pvitEntry = 1
            ...
            if partNotFound:
                ...
                if (no_bch == False):
                    ...
                    pvitShaOffsetStart = (pvitEntry) * pvitRecordSize - pvitSHASize
                    pvitShaOffsetEnd =  (pvitEntry) * pvitRecordSize
                updated_data_buf = data_buf.replace(data_buf[pvitShaOffsetStart:pvitShaOffsetEnd], brBctSha)
                #update usr-ref/target pvit with the updated pvit data
                with open(pvitImg_generated, mode="wb") as fd:
                    if (None == fd.write(updated_data_buf)):
                        self.vlog("Error in writing to  " + pvitImg_generated)
                        AbnormalTermination("Write failed on (%s)" % str(pvitImg_generated), nverror.NvError_FileWriteFailed)
                self.generateSHA512andsignImage(pvitImg_generated, self.targetConfig.s_PVITImgMagicId)
                self.UpdateFileNameinFileToFlash(chain+ "_" + "pvit")
    ...
    # Update signed customer data based on the following, depeneding on the caller
    # flash_bsp_images:
    #     if non-fused board and unsigned images and non-Asymmetric
    # else/bootburn:
    #     if (fused board with signed images) or (non-fused board with unsigned images)
    def IsUpdateSignedCustomerData(self):
        ...
        return updateSignedCustomerData
    ...
    def SetUsbAutoSuspend(self, l_State):
        ...
        if l_State == self.targetConfig.n_UsbAutosuspend.strip():
            self.vlog("The usbAutosuspend is in " + str(l_state))
            return
    ...
    def getListTargetsInRecovery(self, excludeTegraDevice=False):
        ...
        # Multi-die targets assume that the USB devices are sorted by device number
        deviceList.sort()
        return deviceList
    ...
    def BootRCM_Orin_FPGA(self, l_Operation, binaryLocation):
        ...
        mb1_name = bootburnThor.updateImageName(bootburnThor.getMb1NameStem(), True)
