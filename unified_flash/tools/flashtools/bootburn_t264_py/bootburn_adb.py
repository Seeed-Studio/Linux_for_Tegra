...
class bootburn_adb(object):
    ...
    def append_padding_to_file(self, file_path, pad_len, error_context="file"):
        if pad_len <= 0:
            return
        pad_byte = 0xff if self.targetConfig.is_l4t else 0x00
        try:
            with open(file_path, "ab") as f:
                f.write(bytes([pad_byte]) * pad_len)
        except OSError as e:
            self.vlog("Unable to append padding " + str(pad_len) + " bytes (" + str(pad_byte) + ") to " + error_context + ": " + str(e))
    ...
    def FlashUsingADB(self, l_FileName, UpdatePartitions, TempDir, queue):
        ...
        l_BadPageTmp = self.targetConfig.dieId + "_bad-page.tmp"
        ...
        for line in fileData:
            ...
            bct_pattern = r'^(?:[ABC]_)?bct$'
            if (partitionInfo.PartitionName == "A_bct" and self.targetConfig.n_Asymmetric):
                # We only need to merge A_bct with fskp BR BCT in case of asymmetric boot chains
                # merge A_bct with fskp BR BCT
            ...
            elif (re.match(bct_pattern, partitionInfo.PartitionName)):
            ...
            if (partitionInfo.PartitionName == "bct" and self.targetConfig.is_l4t):
                # Replicate BCT in multiple copies
                BCTfileSize = os.stat(partitionInfo.FileName).st_size
                if (BCTfileSize < 16384):
                    ...
                    pad_len = bctBlockSize - BCTfileSize
                    self.log("Make BCT size equal to " + str(bctBlockSize // 1024) + "KB")
                    self.append_padding_to_file(l_BctPartBin, pad_len, "BCT bin")
            ...
            if (partitionInfo.PartitionName == "bad-page"):
                # Create two copies of bad-page for size of the partition.
                # As size of bad page partition is now 512K, each bad page copy is 256K
                # Use device address as prefix for temp filenames to ensure uniqueness
                deviceName = os.path.basename(partitionInfo.LinuxPartitionName)
                devicePrefix = deviceName.split('.')[0]  # Extract address like "810c5b0000" or "1810c5b0000"
                l_BadPagePartBin = devicePrefix + "_badPageBlob.tmp"
                l_BadPageTmp = devicePrefix + "_bad-page.tmp"

                BadPagefileSize = os.stat(PartitionBinary).st_size
                BadPageCopySize = 262144

                if (BadPagefileSize < BadPageCopySize):
                    # Make bad page copy 256K size
                    ...
                    pad_len = BadPageCopySize - BadPagefileSize
                    self.log("Make bad-page size equal to " + str(BadPageCopySize // 1024) + "KB")
                    self.append_padding_to_file(os.path.join(TempDir, l_BadPagePartBin), pad_len, "bad page bin")

                    self.log("Copy resulting file to destination")
                    self.shellUtils.Copy(os.path.join(TempDir, l_BadPagePartBin), os.path.join(TempDir, l_BadPageTmp))
                ...
                start = BadPageCopySize
                fid = open(os.path.join(TempDir, l_BadPageTmp), 'rb')
                ...
                self.shellUtils.removeFile(os.path.join(TempDir, l_BadPageTmp))
