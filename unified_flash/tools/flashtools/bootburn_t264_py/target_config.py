...
class target_config(object):
    ...
    s_HypConfig = None
    mediaCombination = {}
    n_SPE_FW_FOUND = False
    n_SC7_FW_FOUND = False
    ...
    def SetHpseSbPaths(self):
        if (self.isPDK != True):
            if (self.n_EngSample == True):
                self.p_HpseKrnlBin = os.path.join(self.NV_OUTDIR, "nvidia", "spvault", "t264", "integration")
                self.p_HpseCpioBin = os.path.join(self.NV_OUTDIR, "nvidia", "spvault", "t264", "integration")
    ...
    def SetBootBurnInternalSetupPath(self):
        ...
        self.p_PlatformBCT = os.path.join(self.p_Platform, "automotive", "bct")
    ...
    def SetBootBurnPaths(self):
        ...
        if (self.SAFETY_BUILD and self.n_EngSample):
            self.p_HpseBlBin   = os.path.join(self.PDK_TOP, "foundation", "psc_bl1", f"private-{chipFamily}")
            self.p_HpseKrnlBin = os.path.join(self.PDK_TOP, "foundation", "psc_bl1", f"private-{chipFamily}")
            self.p_HpseCpioBin = os.path.join(self.PDK_TOP, "foundation", "psc_bl1", f"private-{chipFamily}")
        else:
        ...
        self.p_HpsePctBin = os.path.join(self.PDK_TOP, self.NV_SDK_NAME_FOUNDATION, "security", "spvault", "out", "nvidia", "spvault", "conf-blob", "t26x")
        ...
        if (self.n_FlashQnx):
            ...
            self.p_StorageDtbPath = os.path.join(self.PDK_TOP, self.NV_SDK_NAME_LINUX, "kernel/nvidia-6.1/preempt_rt")
        ...
        self.p_LinuxStorageDtbPath = os.path.abspath(os.path.join(self.PDK_TOP, self.NV_SDK_NAME_LINUX, "kernel/nvidia-6.1/preempt_rt"))
        self.p_LinuxStorageGenericDtbPath = os.path.abspath(os.path.join(self.PDK_TOP, self.NV_SDK_NAME_LINUX, "kernel/nvidia-6.1/preempt_rt"))
    ...
    def parseDefaultValues(self, variableName, parseVariable=False):
        ...
        path = path.replace("<p_PlatformBCT>", str(self.p_PlatformBCT))
