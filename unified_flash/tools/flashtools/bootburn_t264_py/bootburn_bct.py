...
class StorageConfigParser(object):
    ...
    def preprocessCfgData(self, configData):
        ...
        NOTE: If adding/removing any variable replacement in this function, please update
        OS_ARGS_EXCLUDE_VARIABLES in <TOP>/foundation/tools/makefile_automater/replace_os_args.py
        (OS_ARGS_EXCLUDE_VARIABLES is a list of variables that are not replaced by replace_os_args.py during bind_partitions and those
        variables are meant to be replaced by flash tools in this function)
...
class RcmFlashBctDefines(BctDefines):
    ...
    def _getOperationSpecificDefines(self):
        ...
        if (self.targetConfig.platformConfig):
            ...
            if ('ENABLE_MULTI_SKU_SUPPORT' in self.targetConfig.platformConfig["bct_defines"]["rcm_boot"]):
...
class BootburnBct(object):
    ...
    def __init__(self, targetConfig, outputDir):
        ...
        self.tegraBctArgMap = {
            "--chip": str(targetConfig.GetChipID()) + " " + str(targetConfig.getChipIDMinor(targetConfig.GetChipID()))
        }
        self.targetConfig = targetConfig
    ...
    def generateBct(self, bctDefines):
        ...
        for srcDtsFilePath in dtsFileList:
            ...
            if os.path.abspath(srcDtsFilePath) != os.path.abspath(dstDtsFilePath):
        ...
        parse_error_mode = getattr(self.targetConfig, 'tegrabct_parse_error_mode', None)
        if parse_error_mode is not None:
            if not isinstance(parse_error_mode, str):
                parse_error_mode = str(parse_error_mode)
            parse_error_mode = parse_error_mode.strip()
            if parse_error_mode:
                tegrabctCmd.extend(['--parse-error-mode', parse_error_mode])
        ...
        for tegrabctArg, dtsFile in self.dtsToTegraBctArgMap.items():
            ...
            if (dtsFile):
                ...
                # Generate ratchet blob if --minratchet is present
...
class BootburnBrBct(BootburnBct):
    ...
    """
    def __init__(self, targetConfig, outputDir, bctFileNameSuffix="bct.cfg"):
        ...
        self.bctFileNameSuffix = bctFileNameSuffix
        self.tegraBctArgMap["--brbct"] = self.bctFileNameSuffix
...
class BootburnMb1Bct(BootburnBct):
    ...
    """
    def __init__(self, targetConfig, outputDir, bctFileNameSuffix="bct.cfg", fusebypassName="fusebypass.bin"):
        ...
        if fusebypassName:
            self.tegraBctArgMap["--fb"] = os.path.join(self.outputDir, fusebypassName)

        self.bctFileNameSuffix = bctFileNameSuffix
        self.tegraBctArgMap["--mb1bct"] = bctFileNameSuffix
    ...
    def generateBct(self, bctDefines):
        ...
        if (self.targetConfig.s_BootDevice in ['qspi', 'ufs']):
            ...
            if (storage_devices is not None):
                ...
                cwd = os.getcwd()
                devices_xml = os.path.join(cwd, os.path.basename(storage_devices) + ".xml")
                ...
                storage_devices = os.path.splitext(os.path.basename(devices_xml))[0]
...
class BootburnMb2Bct(BootburnBct):
    ...
    """
    def __init__(self, targetConfig, outputDir, bctFileNameSuffix="mb2bct.cfg"):
