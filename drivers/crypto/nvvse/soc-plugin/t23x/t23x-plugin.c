...
static struct t23x_plugin_data {
	soc_plugin_t23x_dt_data channels[MAX_NUM_IVC];
	uint32_t num_channels;
} g_t23x_plugin_data;
static bool g_gcm_supports_dma;
static struct device *g_gpcdma_dev;
...
Soc_Plugin_Hal_Status soc_populate_dt_data_from_node(const void *nvdt_node)
{
	...
	if (ivc_cnt > MAX_NUM_IVC) {
		 ...
		 */
		if (g_t23x_plugin_data.num_channels > (MAX_NUM_IVC - ivc_cnt)) {
	...
	for (cnt = 0; cnt < ivc_cnt; cnt++) {
		uint32_t index = g_t23x_plugin_data.num_channels;
		...
		g_t23x_plugin_data.channels[index].se_comm_id = se_comm_id;
		...
		g_t23x_plugin_data.channels[index].se_domain = se_domain;
		...
		g_t23x_plugin_data.channels[index].se_domain_instance = se_domain_instance;
		...
		g_t23x_plugin_data.channels[index].se_port = se_port;

		err = of_property_read_u32_index(np, "nvidia,ivccfg", (cnt * TEGRA_IVCCFG_ARRAY_LEN)
						+ TEGRA_VIRTUALIZED_INSTANCE_OFFSET, &virtualized_instance);
		if (err) {
			NVVSE_ERR("%s Error: invalid queue virtualized_instance_number. err %d\n", __func__, err);
			status = SOC_PLUGIN_HAL_FAILED;
			goto exit;
		}
		g_t23x_plugin_data.channels[index].virtualized_instance = virtualized_instance;
		...
		g_t23x_plugin_data.channels[index].mapped_buffer_size = max_buffer_size;
		...
		g_t23x_plugin_data.channels[index].stream_id = sid;

		status = soc_plugin_validate_dt_data(g_t23x_plugin_data.channels[index]);
		...
		g_t23x_plugin_data.num_channels++;
	}
...
int nvvse_soc_plugin_probe(struct platform_device *pdev)
{
	...
	NVVSE_INFO("t23x soc plugin: start probe\n");
	g_gcm_supports_dma = of_property_read_bool(pdev->dev.of_node, "nvidia,gcm-dma-support");

	if (g_gcm_supports_dma)
		g_gpcdma_dev = &pdev->dev;

	NVVSE_INFO("t23x soc plugin: end probe\n");
...
static const struct of_device_id nvvse_soc_plugin_of_match[] = {
	{ .compatible = "nvvse-plug-arch-soc-t23x", },
	{},
};
...
MODULE_DESCRIPTION("NVIDIA Virtual Secure Engine NVVSE SOC T23x Plugin");
