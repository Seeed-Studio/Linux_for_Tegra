...
static const struct host1x_actmon_entry tegra264_actmon_table[] = {
	{
		.classid = HOST1X_CLASS_VIC,
		.name = "vic",
		.irq = 3,
...
static const struct host1x_info host1x09_info = {
	.nb_channels = 63,
	.nb_pts = 1024,
	.nb_mlocks = 24,
	.nb_bases = 0,
	...
	.num_actmon_entries = ARRAY_SIZE(tegra264_actmon_table),
	.actmon_table = tegra264_actmon_table,
	.streamid_vm_table = { 0x1004, 128 },
	.classid_vm_table = { 0x1404, 25 },
	.mmio_vm_table = { 0x1504, 25 },
};
...
static int host1x_probe(struct platform_device *pdev)
{
	...
	/* Create dedicated workqueue for CDMA operations */
	host->cdma_wq = alloc_workqueue("host1x_cdma", 0, 0);
	if (!host->cdma_wq) {
		err = -ENOMEM;
		goto destroy_cache;
	}
...
destroy_wq:
	destroy_workqueue(host->cdma_wq);
destroy_cache:
