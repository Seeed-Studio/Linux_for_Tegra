...
static void pca9685_pwm_set_duty(struct pca9685 *pca, int channel, unsigned int duty)
{
	...
	if (duty == 0) {
		...
		pca9685_write_4reg(&pca->chip, REG_ON_L(channel), (u8[4]){ 0, LED_FULL, 0, 0 });
		return;
	}
	...
	pca9685_write_4reg(&pca->chip, REG_ON_L(channel),
...
static int pca9685_pwm_probe(struct i2c_client *client)
{
	...
	ret = pca9685_write_4reg(&pca->chip, PCA9685_ALL_LED_ON_L, (u8[]){ 0, LED_FULL, 0, LED_FULL });
	if (ret < 0)
