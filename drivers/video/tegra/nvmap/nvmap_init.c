			...
			if (of_device_is_available(it.node)) {
				...
				if (!is_vi_heap) {
					...
					sprintf(name, "ivm%02u%02u%02d", co->vmid, co->peer,
						co->can_alloc);
					pr_info("IVM carveout size=%zu, peer vmid=%u,"
						"name=%s\n",  co->size, co->peer, name);
					co->name      = name;
				}
					...
					struct device *dev)
{
	...
	if (co->cma_dev == NULL) {
		...
		if (!err) {
			...
			pr_err("%s :dma coherent mem declare fail,%zu,err:%d\n",
				co->name, co->size, err);
	} else {
			co->init_done = true;
	}
...
int __init nvmap_init(struct platform_device *pdev)
{
	...
	err = nvmap_init_ivc_queue(&pdev->dev);
