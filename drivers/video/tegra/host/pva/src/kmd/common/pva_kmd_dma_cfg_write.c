				 ...
				 struct pva_fw_dma_descriptor *fw_desc)
{
	...
	fw_desc->srcpt1_cntl =
		assemble_rpt_cntl(desc->src.rpt1, s32_to_u32(desc->src.adv1));
	fw_desc->srcpt2_cntl =
		assemble_rpt_cntl(desc->src.rpt2, s32_to_u32(desc->src.adv2));
	fw_desc->srcpt3_cntl =
		assemble_rpt_cntl(desc->src.rpt3, s32_to_u32(desc->src.adv3));
	fw_desc->dstpt1_cntl =
		assemble_rpt_cntl(desc->dst.rpt1, s32_to_u32(desc->dst.adv1));
	fw_desc->dstpt2_cntl =
		assemble_rpt_cntl(desc->dst.rpt2, s32_to_u32(desc->dst.adv2));
	fw_desc->dstpt3_cntl =
		assemble_rpt_cntl(desc->dst.rpt3, s32_to_u32(desc->dst.adv3));
}
			   ...
			   struct pva_dma_resource_map *dma_resource_map)
{
	...
	for (i = 0; i < dma_cfg->header.num_channels; i++) {
		...
		/* CERT INT34-C: Validation in is_valid_cfg_hdr()
		 * ensures base_channel + num_channels <= n_user_dma_channels + 1 = 16,
		 * so ch_num is guaranteed to be in valid channel range (0-15) */
		ASSERT(ch_num < PVA_KMD_DMA_CHANNELS_PER_ENGINE);
		/* READ/STORE triggers */
		for (j = 0U; j < 7U; j++) {
	...
	*out_fw_fetch_size = (uint32_t)offset;
	...
	for (i = 0U; i < dma_cfg->header.num_descriptors; i++) {
		if (pva_is_reserved_desc(
			...
			// skip over the reserved descriptor
			continue;
		}
