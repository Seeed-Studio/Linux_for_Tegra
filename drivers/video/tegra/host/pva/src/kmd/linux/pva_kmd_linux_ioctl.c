...
static long pva_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	op_err = pva_kmd_ops_handler(
		ocb->kmd_ctx, hdr->mode, &hdr->postfence, ocb->req_buffer,
		(uint32_t)hdr->request.size, ocb->resp_buffer,
		(uint32_t)hdr->response.size, &resp_size, false);
