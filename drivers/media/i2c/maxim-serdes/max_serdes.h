// SPDX-License-Identifier: GPL-2.0
/*
 * Copyright (C) 2023 Analog Devices Inc.
 */

#include <media/v4l2-fwnode.h>
#include <media/v4l2-subdev.h>
#include <linux/pinctrl/pinctrl.h>

#ifndef MAX_SERDES_H
#define MAX_SERDES_H

// #define v4l2_subdev_state v4l2_subdev_pad_config
#ifdef v4l2_subdev_alloc_pad_config
#ifndef v4l2_subdev_alloc_state
#define v4l2_subdev_alloc_state v4l2_subdev_alloc_pad_config
#endif
#endif

#ifdef v4l2_subdev_free_pad_config
#ifndef v4l2_subdev_free_state
#define v4l2_subdev_free_state v4l2_subdev_free_pad_config
#endif
#endif

#ifndef v4l2_async_subdev
#define v4l2_async_subdev v4l2_async_connection
#endif

#ifdef v4l2_async_notifier_init
#ifndef v4l2_async_nf_init
#define v4l2_async_nf_init v4l2_async_notifier_init
#endif
#endif

#ifdef v4l2_async_notifier_unregister
#ifndef v4l2_async_nf_unregister
#define v4l2_async_nf_unregister v4l2_async_notifier_unregister
#endif
#endif

#ifdef v4l2_async_notifier_cleanup
#ifndef v4l2_async_nf_cleanup
#define v4l2_async_nf_cleanup v4l2_async_notifier_cleanup
#endif
#endif

#ifdef v4l2_async_subdev_notifier_register
#ifndef v4l2_async_subdev_nf_register
#define v4l2_async_subdev_nf_register v4l2_async_subdev_notifier_register
#endif
#endif

#ifdef v4l2_async_notifier_add_fwnode_subdev
#ifndef v4l2_async_nf_add_fwnode
#define v4l2_async_nf_add_fwnode(notifier, fwnode, type)		\
	((type *)v4l2_async_notifier_add_fwnode_subdev(notifier, fwnode, sizeof(type)))
#endif
#endif

#define MAX_SERDES_STREAMS_NUM     4
#define MAX_SERDES_VC_ID_NUM	   4

struct max_i2c_xlate {
	u8 src;
	u8 dst;
};

struct max_format {
	const char *name;
	u32 code;
	u8 dt;
	u8 bpp;
	bool dbl;
};

#define MAX_DT_FS			0x00
#define MAX_DT_FE			0x01
#define MAX_DT_EMB8			0x12
#define MAX_DT_YUV422_8B		0x1e
#define MAX_DT_YUV422_10B		0x1f
#define MAX_DT_RGB565			0x22
#define MAX_DT_RGB666			0x23
#define MAX_DT_RGB888			0x24
#define MAX_DT_RAW8			0x2a
#define MAX_DT_RAW10			0x2b
#define MAX_DT_RAW12			0x2c
#define MAX_DT_RAW14			0x2d
#define MAX_DT_RAW16			0x2e
#define MAX_DT_RAW20			0x2f

const struct max_format *max_format_by_index(unsigned int index);
const struct max_format *max_format_by_code(u32 code);
const struct max_format *max_format_by_dt(u8 dt);

#endif // MAX_SERDES_H
