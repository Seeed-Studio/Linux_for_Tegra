...
#define CR_TIMER 1
void dump_cr(_adapter *a)
{
	u32 cr_1a24 = 0, cr_1a34 = 0, cr_1a64 = 0, cr_1a08 = 0;
	u32 rf_0a = 0, rf_0b = 0, rf_18a = 0, rf_18b = 0;

	cr_1a24 = rtw_phl_read32(a->dvobj->phl, 0x11a24);
	cr_1a34 = rtw_phl_read32(a->dvobj->phl, 0x11a34);
	cr_1a64 = rtw_phl_read32(a->dvobj->phl, 0x11a64);
	cr_1a08 = rtw_phl_read32(a->dvobj->phl, 0x11a08);
	rf_0a = rtw_phl_read_rfreg(a->dvobj->phl, RF_PATH_A, 0x0 , 0xFFFFF);
	rf_0b = rtw_phl_read_rfreg(a->dvobj->phl, RF_PATH_B, 0x0 , 0xFFFFF);
	rf_18a = rtw_phl_read_rfreg(a->dvobj->phl, RF_PATH_A, 0x18 , 0xFFFFF);
	rf_18b = rtw_phl_read_rfreg(a->dvobj->phl, RF_PATH_B, 0x18 , 0xFFFFF);

	printk("1A24=0x%08x, 1A34=0x%08x, 1A64=0x%08x, 1A08=0x%08x\n",
		cr_1a24, cr_1a34, cr_1a64, cr_1a08);
	printk("RF0=0x%05x:0x%05x, RF18=0x%05x:0x%05x\n",
		rf_0a, rf_0b, rf_18a, rf_18b);
}
...
void init_mlme_ext_timer(_adapter *padapter)
{
	...
	rtw_init_timer(&pmlmeext->cr_timer, cr_timer_hdl, padapter);
		...
		const NDIS_802_11_SSID *pssid, const u8 *da, u8 ch, bool append_wps, int wait_ack)
{
	...
	if (da && pssid)
		RTW_INFO(FUNC_ADPT_FMT" to %s ("MAC_FMT") ch:%u\n",
			FUNC_ADPT_ARG(padapter), pssid->Ssid, MAC_ARG(da), rtw_get_oper_ch(padapter, padapter_link));
	else if (pssid)
		RTW_INFO(FUNC_ADPT_FMT" to %s (ff:ff:ff:ff:ff:ff) ch:%u\n",
			FUNC_ADPT_ARG(padapter), pssid->Ssid, rtw_get_oper_ch(padapter, padapter_link));
	else
...
static u8 * rtw_set_rtk_version_ie(_adapter *padapter , u8 *pframe ,uint *frlen ) {
	int vender_len;
	systime current_time = rtw_get_current_time();
	unsigned char vendor_info[255];
	unsigned char REALTEK_OUI[] = {0x00, 0xe0, 0x4c, 0xff};
	unsigned char *ptr;

	_rtw_memset(vendor_info,0,255);
	_rtw_memcpy(vendor_info, REALTEK_OUI, 4);
	ptr = vendor_info + 4;
	snprintf(ptr , sizeof(vendor_info) - 4, "%s-%ld", DRIVERVERSION, current_time);
	vender_len = strlen(ptr) + 4;

	RTW_INFO("version IE %s\n", ptr);
			...
			u8 *pframe, struct pkt_attrib *pattrib, u8 is_reassoc)
{
	...
	dump_cr(padapter);
	pmlmeext->cr_timer_cnt = 30; /* dump max 30 times */
	_set_timer(&pmlmeext->cr_timer, CR_TIMER);
...
#ifdef DBG_ROAMING_TEST
	pframe = rtw_set_rtk_version_ie(padapter, pframe, &pattrib->pktlen);
#endif
...
void start_clnt_join(_adapter *padapter)
{
	...
	if (caps & cap_ESS) {
		...
		if (padapter->mlmepriv.roam_network) {
			rtw_wnm_start_clnt_join(padapter, padapter_link);
		}
		else
#endif
		{
...
unsigned int receive_disconnect(_adapter *padapter, unsigned char *MacAddr, unsigned short reason, u8 locally_generated)
{
	...
	if ((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) {
...
#endif
#ifdef CONFIG_RTW_FSM
		rtw_fsm_notify_disconnect_start(&padapter->fsmpriv);
#endif
		if (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS) {
		...
		rtw_signal_state_reset(padapter);
#endif
	}
...
u32 report_join_res(_adapter *padapter, int aid_res, u16 status)
{
	...
	struct link_mlme_ext_priv *pmlmeext = &padapter_link->mlmeextpriv;
	struct link_mlme_ext_info *pmlmeinfo = &(pmlmeext->mlmext_info);
	struct mlme_ext_priv *pmlmeext_ = &padapter->mlmeextpriv;
	struct cmd_priv *pcmdpriv = &adapter_to_dvobj(padapter)->cmdpriv;
	u32 ret = _FAIL;
#ifdef CONFIG_80211BE_EHT
	...
	u8 lidx;
#endif
	_cancel_timer_ex(&pmlmeext_->cr_timer);
...
void cr_timer_hdl(void *context)
{
