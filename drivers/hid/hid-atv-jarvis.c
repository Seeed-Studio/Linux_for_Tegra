...
/* counter of how many continuous silent timer callback in a row */
...
static int atomic_fifo_init(struct simple_atomic_fifo *fifo_ptr, uint size)
{
	...
	/* Ensure data is written before status flag is updated */
	smp_wmb();
	return 0;
}
...
{
	/* Ensure data is read before status flag is updated */
	smp_rmb();
		...
		uint frames)
{
	/* Ensure data is read before status flag is updated */
	smp_rmb();
	WARN_ON(frames > atomic_fifo_available_to_read(fifo_ptr));
		...
		uint frames)
{
	/* Ensure data is read before status flag is updated */
	smp_rmb();
	WARN_ON(frames > atomic_fifo_available_to_write(fifo_ptr));
...
static void snd_atvr_timer_callback(struct timer_list *t)
{
	...
	/*
	 * NEW: Get 'atvr_snd' from the timer 't' using the correct member name.
	 */
	struct snd_atvr *atvr_snd = from_timer(atvr_snd, t, decoding_timer);

	/*
	 * NEW: Get 'substream' from the 'atvr_snd' struct.
	 * This now works because you added it in Action 1.
	 */
	struct snd_pcm_substream *substream = atvr_snd->substream;
	...
	if (silence_counter > MAX_SILENCE_COUNTER) {
		spin_unlock_irqrestore(&atvr_snd->timer_lock, flags);
		pr_info_ratelimited("max silence timeout reached.\n");
		return;
	}
	spin_unlock_irqrestore(&atvr_snd->timer_lock, flags);
...
static int snd_atvr_pcm_copy(struct snd_pcm_substream *substream,
			...
			struct iov_iter *iter, snd_pcm_uframes_t count)
#else
static int snd_atvr_pcm_copy(struct snd_pcm_substream *substream,
...
#endif
{
	...
	if ((pos + count) > atvr_snd->frames_per_buffer) {
		...
		size_t num_bytes = num_frames * sizeof(int16_t);
#if KERNEL_VERSION(6, 6, 0) <= LINUX_VERSION_CODE
		if (copy_to_iter(source, num_bytes, iter) != num_bytes)
#else
			     ...
			     int substreams)
{
	...
	pcm->info_flags = 0;
	strscpy(pcm->name, "SHDR PCM", sizeof(pcm->name));
			...
			struct snd_card **p_shdr_card)
{
	...
	*p_shdr_card = shdr_card;
	...
	strscpy(shdr_card->driver, "SHIELD Rmt Aud", sizeof(shdr_card->driver));
	strscpy(shdr_card->shortname, "SHDRAudio",
...
static void atvr_remove(struct hid_device *hdev)
{
	...
	if (hdev->product == USB_DEVICE_ID_NVIDIA_THUNDERSTRIKE && hdev->uniq[0]) {
