...
bool de_statue;
 ...
 static void tegra_uart_tx_dma_complete(void *args)
 {
	 ...
	 if (tup->tx_in_progress == 0 && uart_circ_empty(xmit)) {
 ...
 {
	 struct circ_buf *xmit = &tup->uport.state->xmit;
	 printk("in tegra_uart_handle_tx_pio\r\n");
 ...
 static enum hrtimer_restart tegra_uart_rs485_de_hrtimer_callback(struct hrtimer *timer)
{
	struct tegra_uart_port *tup = container_of(timer, struct tegra_uart_port, rs485_de_timer);
	struct uart_port *u = &tup->uport;
	unsigned long lsr;
	...
	if (lsr & UART_LSR_TEMT) {
		tegra_uart_rs485_de_deassert(tup);
		dev_info(u->dev, "RS485: DE released (hrtimer)\n");
 ...
 static int tegra_uart_probe(struct platform_device *pdev)
 {
	 ...
	 tup->rs_485_gpio_statu = false;
	 tup->rs485_de_gpio = NULL;
	 tup->rs485_de_gpio = devm_gpiod_get_optional(&pdev->dev, "rs485-de", GPIOD_OUT_HIGH);
	 if (tup->rs485_de_gpio == NULL) {
		dev_err(&pdev->dev, "Failed to get RS485 DE gpio\n");
	 }else{
		dev_info(&pdev->dev, "RS485 DE gpio acquired, default LOW (recv mode)\n");
		hrtimer_init(&tup->rs485_de_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
		tup->rs485_de_timer.function = tegra_uart_rs485_de_hrtimer_callback;
	 }
