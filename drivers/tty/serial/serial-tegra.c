...
static void tegra_uart_handle_tx_pio(struct tegra_uart_port *tup)
{
	...
	tegra_uart_start_next_tx(tup);
	if (uart_circ_empty(xmit) && tup->rs485_de_gpio)
		hrtimer_start(&tup->rs485_de_timer,
			      ns_to_ktime(10000), HRTIMER_MODE_REL);
}
...
static int tegra_uart_probe(struct platform_device *pdev)
{
	...
	if (tup->rs485_de_gpio) {
		dev_info(&pdev->dev, "RS485 DE gpio acquired, default HIGH (recv mode)\n");
