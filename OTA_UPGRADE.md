...
Advantages: preserves user configuration and data, does not trigger the initial setup wizard. Custom kernel packages are ~49MB; official userspace packages are downloaded from apt.
...
```bash
scp ../ota_packages/*.deb nvidia@<JETSON_IP>:~/ota-debs/
```

## Step 3: Upgrade on device

```bash
# 1. Unlock held packages
...
## 7b. Remove disable flag and reinstall official bootloader
...
**Note**: The TNSPEC replacement in step 7a depends on the product:
...
Alternatively, use the included helper script after step 6:
```bash
sudo bash ~/ota-debs/update_uefi.sh
```
