...
RELEASE_FILE="${KERNEL_OUT}/kernel/kernel-jammy-src/include/config/kernel.release"
...
RT_IMAGE="${RT_IMAGE:-}"
if [[ -z "${RT_IMAGE}" ]]; then
	RT_IMAGE="${KERNEL_OUT}/kernel/kernel-jammy-src/arch/arm64/boot/Image"
	if [[ ! -f "${RT_IMAGE}" ]]; then
...
# the initrd; the RT tree is silently skipped.  Remove the stray rm before
# rebuilding the initrd.
if [ -f /usr/sbin/nv-update-extlinux ] && grep -q 'copy_files_initrd "\${DESTDIR}" "\${temp_list}" "\${_kernel_version}"' /usr/sbin/nv-update-initrd 2>/dev/null; then
