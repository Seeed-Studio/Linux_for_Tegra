...
function check_pre_req()
{
	...
	if [ "${board}" == "" ]; then
		echo "ERROR: Invalid board name" >&2
		usage
	else
		case "${board}" in
		jetson-xavier-nx-devkit)
	...
	if [ "${sd_blob_name}" == "" ]; then
		echo "ERROR: Invalid SD blob image name" >&2
		usage
	fi

	if [ ! -f "${l4t_dir}/flash.sh" ]; then
		echo "ERROR: ${l4t_dir}/flash.sh is not found" >&2
		usage
	fi

	if [ ! -f "${l4t_tools_dir}/nvptparser.py" ]; then
		echo "ERROR: ${l4t_tools_dir}/nvptparser.py is not found" >&2
		usage
	fi

	if [ ! -d "${bootloader_dir}" ]; then
		echo "ERROR: ${bootloader_dir} directory not found" >&2
		usage
	fi
...
function create_signed_images()
{
	...
	if [ ! -f "${bootloader_dir}/flashcmd.txt" ]; then
		echo "ERROR: ${bootloader_dir}/flashcmd.txt not found" >&2
		exit 1
	fi

	if [ ! -d "${signed_image_dir}" ]; then
		echo "ERROR: ${bootloader_dir}/signed directory not found" >&2
		exit 1
	fi
	...
	if [ ! -f "${signed_image_dir}/${signed_cfg}" ]; then
		echo "ERROR: ${signed_image_dir}/${signed_cfg} not found" >&2
		exit 1
	fi
}
