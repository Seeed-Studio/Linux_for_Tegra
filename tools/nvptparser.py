...
def do_parse(config_file, storage):
    ...
    for p in tree.findall("./device[@type='%s']/partition" % storage):
        ...
        if name == 'master_boot_record':
            mbr = (name, size, filename)
            continue

        if name == 'APP':
            partitions.insert(0, (name, size, filename))
        else:
            partitions.append((name, size, filename))

    if mbr:
        partitions.append(mbr)

    result = []
    for i, (name, size, filename) in enumerate(partitions):
...
if __name__ == '__main__':
    ...
    for (i, n, s, f) in result:
        print('part_num=%d;part_name=%s;part_size=%s;part_file=%s' % (i, n, s, f))
