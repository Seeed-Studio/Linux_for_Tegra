...
endpoint_fops_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	vma->vm_pgoff  = 0;
#if defined(NV_VM_FLAGS_SET_PRESENT) /* Linux v6.3 */
		   ...
		   struct endpoint_t *endpoint)
{
	...
	if (ret < 0) {
		pr_err("(%s): Failed to fetch shim info\n", endpoint->name);
		goto err;
	}

	syncpt->phy_addr = (base + ((u64)syncpt->id * stride));
...
allocate_memory(struct endpoint_drv_ctx_t *eps_ctx, struct endpoint_t *ep)
{
	...
	ret = pci_client_map_addr(ep->pci_client_h, ep->iova,
				  ...
				  prot);
	if (ret) {
		pr_err("(%s): Failed to map physical page to reserved iova, ret = (%d)\n",
		       ep->name, ret);
		goto err;
	}
	ep->mapped_iova = true;
