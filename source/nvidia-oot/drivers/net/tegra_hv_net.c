...
#define DEFAULT_CARRIER_RETRY_DELAY_MSECS	1000
...
struct tegra_hv_net {
	...
	struct delayed_work carrier_retry_work;
	unsigned int carrier_retry_delay;
...
static void tegra_hv_net_carrier_retry_work(struct work_struct *work)
{
	struct tegra_hv_net *hvn =
		container_of(work, struct tegra_hv_net, carrier_retry_work.work);

	if (tegra_hv_ivc_can_write(hvn->ivck)) {
		netif_carrier_on(hvn->ndev);
		netdev_info(hvn->ndev, "IVC write channel available, carrier restored\n");
	} else {
		/* Schedule retry after delay */
		queue_delayed_work(hvn->xmit_wq, &hvn->carrier_retry_work,
				   msecs_to_jiffies(hvn->carrier_retry_delay));
	}
}
...
static void *tegra_hv_net_xmit_get_buffer(struct tegra_hv_net *hvn)
{
	...
	if (IS_ERR(p)) {
		...
		if (ret <= 0) {
			netif_carrier_off(hvn->ndev);

			/* Flush the TX queue since the data isn't useful anymore:
			 * - If peer is not up yet, packets will fail on 1st attempt anyway
			 * - If it's VM reset, the queued data is stale
			 * - No new TX requests will come until carrier is back on
			 */
			skb_queue_purge(&hvn->tx_q);
			queue_delayed_work(hvn->xmit_wq, &hvn->carrier_retry_work,
					   msecs_to_jiffies(hvn->carrier_retry_delay));
		}
	}
...
static void tegra_hv_net_xmit_work(struct work_struct *work)
{
	...
	while ((skb = skb_dequeue(&hvn->tx_q)) != NULL) {
		...
		if (ret != 0) {
			netdev_err(hvn->ndev, "%s: skb_linearize error=%d\n",
				   __func__, ret);
...
static int tegra_hv_net_stop(struct net_device *ndev)
{
	struct tegra_hv_net *hvn = netdev_priv(ndev);
...
static void tegra_hv_net_get_stats64(struct net_device *ndev,
					     ...
					     struct ethtool_drvinfo *info)
{
	...
	strscpy(info->bus_info, dev_name(&hvn->pdev->dev),
		sizeof(info->bus_info));
}

/* Enum for ethtool statistics indices - ensures alignment */
enum tegra_hv_net_stat_index {
	TEGRA_HV_NET_STAT_TX_PACKETS,
	TEGRA_HV_NET_STAT_TX_BYTES,
	TEGRA_HV_NET_STAT_TX_DROPS,
	TEGRA_HV_NET_STAT_TX_LINEARIZE_FAIL,
	TEGRA_HV_NET_STAT_TX_QUEUE_FULL,
	TEGRA_HV_NET_STAT_TX_WQ_FAIL,
	TEGRA_HV_NET_STAT_TX_IVC_WRITE_FAIL,
	TEGRA_HV_NET_STAT_RX_PACKETS,
	TEGRA_HV_NET_STAT_RX_BYTES,
	TEGRA_HV_NET_STAT_RX_DROPS,
	TEGRA_HV_NET_STAT_RX_BAD_FRAME,
	TEGRA_HV_NET_STAT_RX_BAD_PACKET,
	TEGRA_HV_NET_STAT_RX_UNEXPECTED_PACKET,
	TEGRA_HV_NET_STAT_RX_ALLOC_FAIL,
	TEGRA_HV_NET_STAT_RX_OVERFLOW,
	TEGRA_HV_NET_STAT_RX_NEXT_FRAME,
	TEGRA_HV_NET_STAT_NAPI_SCHEDULES,
	TEGRA_HV_NET_STAT_COUNT /* Must be last - used for array size */
};

static const char *const tegra_hv_net_stat_names[] = {
	[TEGRA_HV_NET_STAT_TX_PACKETS] = "tx_packets",
	[TEGRA_HV_NET_STAT_TX_BYTES] = "tx_bytes",
	[TEGRA_HV_NET_STAT_TX_DROPS] = "tx_drops",
	[TEGRA_HV_NET_STAT_TX_LINEARIZE_FAIL] = "tx_linearize_fail",
	[TEGRA_HV_NET_STAT_TX_QUEUE_FULL] = "tx_queue_full",
	[TEGRA_HV_NET_STAT_TX_WQ_FAIL] = "tx_wq_fail",
	[TEGRA_HV_NET_STAT_TX_IVC_WRITE_FAIL] = "tx_ivc_write_fail",
	[TEGRA_HV_NET_STAT_RX_PACKETS] = "rx_packets",
	[TEGRA_HV_NET_STAT_RX_BYTES] = "rx_bytes",
	[TEGRA_HV_NET_STAT_RX_DROPS] = "rx_drops",
	[TEGRA_HV_NET_STAT_RX_BAD_FRAME] = "rx_bad_frame",
	[TEGRA_HV_NET_STAT_RX_BAD_PACKET] = "rx_bad_packet",
	[TEGRA_HV_NET_STAT_RX_UNEXPECTED_PACKET] = "rx_unexpected_packet",
	[TEGRA_HV_NET_STAT_RX_ALLOC_FAIL] = "rx_alloc_fail",
	[TEGRA_HV_NET_STAT_RX_OVERFLOW] = "rx_overflow",
	[TEGRA_HV_NET_STAT_RX_NEXT_FRAME] = "rx_next_frame",
	[TEGRA_HV_NET_STAT_NAPI_SCHEDULES] = "napi_schedules",
};

/* Compile-time check to ensure array size matches enum count */
static_assert(ARRAY_SIZE(tegra_hv_net_stat_names) == TEGRA_HV_NET_STAT_COUNT,
	      "tegra_hv_net_stat_names array size must match TEGRA_HV_NET_STAT_COUNT");

#define TEGRA_HV_NET_NUM_STATS TEGRA_HV_NET_STAT_COUNT

static int tegra_hv_net_get_sset_count(struct net_device *ndev, int sset)
{
	if (sset == ETH_SS_STATS)
		return TEGRA_HV_NET_NUM_STATS;
	return 0;
}

static void tegra_hv_net_get_strings(struct net_device *ndev, u32 sset,
				     u8 *data)
{
	unsigned int i;
	...
	/* Copy each stat name into the fixed-size ethtool string slots */
	for (i = 0; i < TEGRA_HV_NET_NUM_STATS; i++)
		strscpy((char *)data + i * ETH_GSTRING_LEN,
			tegra_hv_net_stat_names[i], ETH_GSTRING_LEN);
}

static void tegra_hv_net_get_ethtool_stats(struct net_device *ndev,
					   struct ethtool_stats *stats,
					   u64 *data)
{
	struct tegra_hv_net *hvn = netdev_priv(ndev);
	u64 tx_packets = 0, tx_bytes = 0, tx_drops = 0;
	u64 tx_linearize_fail = 0, tx_queue_full = 0, tx_wq_fail = 0,
	    tx_ivc_write_fail = 0;
	u64 rx_packets = 0, rx_bytes = 0, rx_drops = 0;
	u64 rx_bad_frame = 0, rx_bad_packet = 0, rx_unexpected_packet = 0,
	    rx_alloc_fail = 0, rx_overflow = 0, rx_next_frame = 0;
	u64 napi_schedules = 0;
	int cpu;

	for_each_possible_cpu(cpu) {
		struct tegra_hv_net_stats *s = per_cpu_ptr(hvn->stats, cpu);
		u64 start;

		do {
			start = u64_stats_fetch_begin(&s->tx_syncp);
			tx_packets += s->tx_packets;
			tx_bytes += s->tx_bytes;
			tx_drops += s->tx_drops;
			tx_linearize_fail += s->tx_linearize_fail;
			tx_queue_full += s->tx_queue_full;
			tx_wq_fail += s->tx_wq_fail;
			tx_ivc_write_fail += s->tx_ivc_write_fail;
		} while (u64_stats_fetch_retry(&s->tx_syncp, start));
		do {
			start = u64_stats_fetch_begin(&s->rx_syncp);
			rx_packets += s->rx_packets;
			rx_bytes += s->rx_bytes;
			rx_drops += s->rx_drops;
			rx_bad_frame += s->rx_bad_frame;
			rx_bad_packet += s->rx_bad_packet;
			rx_unexpected_packet += s->rx_unexpected_packet;
			rx_alloc_fail += s->rx_alloc_fail;
			rx_overflow += s->rx_overflow;
			rx_next_frame += s->rx_next_frame;
			napi_schedules += s->napi_schedules;
		} while (u64_stats_fetch_retry(&s->rx_syncp, start));
	}

	data[TEGRA_HV_NET_STAT_TX_PACKETS] = tx_packets;
	data[TEGRA_HV_NET_STAT_TX_BYTES] = tx_bytes;
	data[TEGRA_HV_NET_STAT_TX_DROPS] = tx_drops;
	data[TEGRA_HV_NET_STAT_TX_LINEARIZE_FAIL] = tx_linearize_fail;
	data[TEGRA_HV_NET_STAT_TX_QUEUE_FULL] = tx_queue_full;
	data[TEGRA_HV_NET_STAT_TX_WQ_FAIL] = tx_wq_fail;
	data[TEGRA_HV_NET_STAT_TX_IVC_WRITE_FAIL] = tx_ivc_write_fail;
	data[TEGRA_HV_NET_STAT_RX_PACKETS] = rx_packets;
	data[TEGRA_HV_NET_STAT_RX_BYTES] = rx_bytes;
	data[TEGRA_HV_NET_STAT_RX_DROPS] = rx_drops;
	data[TEGRA_HV_NET_STAT_RX_BAD_FRAME] = rx_bad_frame;
	data[TEGRA_HV_NET_STAT_RX_BAD_PACKET] = rx_bad_packet;
	data[TEGRA_HV_NET_STAT_RX_UNEXPECTED_PACKET] = rx_unexpected_packet;
	data[TEGRA_HV_NET_STAT_RX_ALLOC_FAIL] = rx_alloc_fail;
	data[TEGRA_HV_NET_STAT_RX_OVERFLOW] = rx_overflow;
	data[TEGRA_HV_NET_STAT_RX_NEXT_FRAME] = rx_next_frame;
	data[TEGRA_HV_NET_STAT_NAPI_SCHEDULES] = napi_schedules;
}

static const struct ethtool_ops tegra_hv_ethtool_ops = {
	.get_drvinfo = tegra_hv_net_ethtool_get_drvinfo,
	.get_link = ethtool_op_get_link,
	.get_strings = tegra_hv_net_get_strings,
	.get_ethtool_stats = tegra_hv_net_get_ethtool_stats,
	.get_sset_count = tegra_hv_net_get_sset_count,
};
...
static int tegra_hv_net_rx(struct tegra_hv_net *hvn, int limit)
{
	...
	while (nr < limit) {
		...
		if (!hvn->rx_skb) {
			...
			hvn->rx_skb = napi_alloc_skb(&hvn->napi, count);
			if (!hvn->rx_skb) {
		...
		/* append the data */
		skb = hvn->rx_skb;
		skb_put_data(skb, p + 2, frame_size);
		...
		if (last) {
			...
			/* Mark checksum as unnecessary since packets are transferred
			 * through trusted hypervisor memory, not physical wire.
			 * This avoids expensive software checksum verification.
			 */
			skb->ip_summed = CHECKSUM_UNNECESSARY;

			napi_gro_receive(&hvn->napi, skb);
			hvn->rx_skb = NULL;
		}
		dk = dk_none;
drop:
...
drop_and_exit:
	/* Update stats for the failed frame and exit loop */
	u64_stats_update_begin(&stats->rx_syncp);
	stats->rx_drops++;
	stats->rx_next_frame++;
	u64_stats_update_end(&stats->rx_syncp);
...
static int tegra_hv_net_poll(struct napi_struct *napi, int budget)
{
	...
	/* Count actual NAPI poll executions */
	u64_stats_update_begin(&stats->rx_syncp);
	stats->napi_schedules++;
	u64_stats_update_end(&stats->rx_syncp);
...
static int tegra_hv_net_probe(struct platform_device *pdev)
{
	...
	int ret;
	u32 id;
	u32 highmark, lowmark, txdelay, carrier_retry_delay;
	...
	if (highmark <= lowmark) {
		dev_err(dev,
			"Bad watermark configuration (high <= low = %u < %u)\n",
			highmark, lowmark);
		goto out_of_put;
	}
	...
	ret = of_property_read_u32(dn, "carrier-retry-delay-msecs",
				   &carrier_retry_delay);
	if (ret)
		carrier_retry_delay = DEFAULT_CARRIER_RETRY_DELAY_MSECS;
	...
	dev_err(dev, "Reserved IVC channel #%d - frame_size=%d\n", id,
		hvn->ivck->frame_size);
	...
	INIT_DELAYED_WORK(&hvn->carrier_retry_work, tegra_hv_net_carrier_retry_work);
	hvn->carrier_retry_delay = carrier_retry_delay;
	 ...
	 * Since packets are transferred through trusted hypervisor shared memory
	 * (not physical wire), there is no risk of corruption and checksums are
	 * already validated by the source VM. We can safely skip checksum
	 * verification to improve performance, especially for small packets.
	 */
	...
	if (of_property_read_bool(dev->of_node, "use-random-mac-addr")) {
		...
		dev_warn(dev,
			 "No valid mac-address found, using fixed local address\n");
		...
		addr[5] = (uint8_t)(gid);
		dev_addr_set(ndev, (const u8 *)&addr);
	}
...
static int tegra_hv_net_remove(struct platform_device *pdev)
{
	...
	unregister_netdev(ndev);
	netif_napi_del(&hvn->napi);
	cancel_delayed_work_sync(&hvn->carrier_retry_work);
	destroy_workqueue(hvn->xmit_wq);
	tegra_hv_ivc_unreserve(hvn->ivck);
	free_percpu(hvn->stats);
	free_netdev(ndev);
...
static void tegra_hv_net_remove_wrapper(struct platform_device *pdev)
{
	tegra_hv_net_remove(pdev);
}
#else
static int tegra_hv_net_remove_wrapper(struct platform_device *pdev)
{
	return tegra_hv_net_remove(pdev);
}
#endif
...
static struct platform_driver tegra_hv_net_driver = {
	...
	.remove	= tegra_hv_net_remove_wrapper,
#ifdef CONFIG_PM_SLEEP
...
MODULE_AUTHOR("Sarangdhar Joshi <sarangj@nvidia.com>");
