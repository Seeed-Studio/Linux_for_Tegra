		  ...
		  struct bb_plcp_cr_info *cr, enum phl_phy_idx phy_idx)
{
	...
	else if (bb->ic_type == BB_RTL8852C && bb->ic_sub_type != BB_IC_SUB_TYPE_8852C_8852D) {
		pw_a_cr = 0x5428;
		pw_b_cr = 0x7428;
		    ...
		    char *output, u32 *_out_len)
{
	...
	if (_os_strcmp(input[2], "ppdu_type") == 0) {
		...
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->mode = (u8)val[0];
	} else if (_os_strcmp(input[2], "max_tx_time") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
	...
	} else if (_os_strcmp(input[2], "ru_tot_sts_max") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
	...
	} else if (_os_strcmp(input[2], "tb_pre_fec_pad") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
		in->tb_pre_fec_padding_factor = (u8)val[0];
	} else if (_os_strcmp(input[2], "l_preamble_en") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
	...
	}else if (_os_strcmp(input[2], "tb_mumimo_en") == 0) {
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[0]);
