...
int vblk_complete_rpmb_req(struct vblk_dev *vblkdev,
...
int vblk_prep_rpmb_req(struct vblk_dev *vblkdev,
		...
		struct vsc_request *vsc_req)
{
	...
	vs_req->blkdev_req.req_op = VS_BLK_RPMB;
	memcpy(vsc_req->mempool_virt, ioctl_req->ioctl_buf,
			ioctl_req->ioctl_len);
	vs_req->blkdev_req.rpmb_req.rpmb_op_id = ioctl_req->ioctl_id;
	vs_req->blkdev_req.rpmb_req.rpmb_len = ioctl_req->ioctl_len;
	vs_req->blkdev_req.rpmb_req.rpmb_data_offset = vsc_req->mempool_offset;
...
int vblk_prep_rpmb_io(struct vblk_dev *vblkdev,
		struct vblk_ioctl_req *ioctl_req,
		void __user *user)
{
	int err = 0;
	...
	struct vblk_rpmb_io_hdr *vblk_rpmb_hp;
	uint32_t vblk_rpmb_header_len = sizeof(struct vblk_rpmb_io_hdr);
	uint32_t sbp_offset;
	void *sbp;
	...
	sbp_offset = vblk_rpmb_header_len;
	...
	cdb_buf = kzalloc(hp->cmd_len, GFP_KERNEL);
	if (cdb_buf == NULL) {
		err = -ENOMEM;
		goto free_hp;
	}

	if (copy_from_user(cdb_buf, hp->cmdp, hp->cmd_len)) {
		err = -EFAULT;
		goto free_ioctl_buf;
	}
	...
	vblk_rpmb_hp = (struct vblk_rpmb_io_hdr *)(ioctl_buf);
	memcpy(vblk_rpmb_hp->cdb, cdb_buf,
	hp->cmd_len > SCSI_RPMB_HDR_LEN ? SCSI_RPMB_HDR_LEN : hp->cmd_len);
	data_buf = (ioctl_buf + data_buf_offset_aligned);

	switch (hp->dxfer_direction) {
	case SG_DXFER_NONE:
			vblk_rpmb_hp->data_direction = SCSI_DATA_NONE;
			break;
	case SG_DXFER_TO_DEV:
			vblk_rpmb_hp->data_direction = SCSI_TO_DEVICE;
			break;
	case SG_DXFER_FROM_DEV:
			vblk_rpmb_hp->data_direction = SCSI_FROM_DEVICE;
			break;
	case SG_DXFER_TO_FROM_DEV:
			vblk_rpmb_hp->data_direction = SCSI_BIDIRECTIONAL;
			break;
	default:
			err = -EBADMSG;
			goto free_ioctl_buf;
	...
	if ((vblk_rpmb_hp->data_direction == SCSI_TO_DEVICE) ||
		(vblk_rpmb_hp->data_direction == SCSI_BIDIRECTIONAL)) {
	...
	vblk_rpmb_hp->mx_sb_len = max_sb_len;
	/* This is actual data len on which storage server needs to act */
	vblk_rpmb_hp->dxfer_len = hp->dxfer_len;
	 ...
	 */
	vblk_rpmb_hp->dxfer_buf_len = data_buf_size_aligned;
	vblk_rpmb_hp->xfer_arg_offset = data_buf_offset_aligned;
	vblk_rpmb_hp->sbp_arg_offset = sbp_offset;
	ioctl_req->ioctl_id = UFS_RPMB_IOCTL_ID;
...
int vblk_complete_rpmb_io(struct vblk_dev *vblkdev,
		struct vblk_ioctl_req *ioctl_req,
		void __user *user)
{
	sg_io_hdr_t *hp = NULL;
	...
	if (!ioctl_req->ioctl_buf ||
			ioctl_req->ioctl_len < sizeof(struct vblk_rpmb_io_hdr)) {
		err = -EINVAL;
		goto exit;
	}
	...
	vblk_rpmb_hp = (struct vblk_rpmb_io_hdr *)(ioctl_req->ioctl_buf);
	/* Validate sense buffer length */
	safe_sb_len = min_t(uint32_t, hp->mx_sb_len, UFS_REQUEST_SENS_DATA_LEN);

	hp->status = 0xff & vblk_rpmb_hp->status;
	hp->masked_status = status_byte(vblk_rpmb_hp->status);
	hp->host_status = host_byte(vblk_rpmb_hp->status);
	hp->driver_status = driver_byte(vblk_rpmb_hp->status);

	if (hp->mx_sb_len != 0U)
		hp->sb_len_wr = min_t(uint32_t, vblk_rpmb_hp->sb_len_wr, safe_sb_len);
	else
		hp->sb_len_wr = 0U;
	...
	sbp = (ioctl_req->ioctl_buf + vblk_rpmb_hp->sbp_arg_offset);

	/* Validate sense buffer parameters before copying */
	if ((hp->sb_len_wr != 0) && (hp->sbp != NULL)) {
		if (hp->sb_len_wr > safe_sb_len) {
			err = -EINVAL;
			goto free_hp;
		}
	...
	data_buf = (ioctl_req->ioctl_buf + vblk_rpmb_hp->xfer_arg_offset);

	/* Validate data transfer */
	if ((vblk_rpmb_hp->data_direction == SCSI_FROM_DEVICE) ||
		(vblk_rpmb_hp->data_direction == SCSI_BIDIRECTIONAL)) {
		if (vblk_rpmb_hp->dxfer_len > ioctl_req->ioctl_len) {
			err = -EINVAL;
			goto free_hp;
		}
		if (copy_to_user(hp->dxferp, data_buf, vblk_rpmb_hp->dxfer_len)) {
			err = -EFAULT;
			goto free_hp;
		}
	}
