 ...
 * Display stream IDs:
 * Notes:
 * 1. SID[7:0] 127 is a Bypass SID.
 * 2. Display HW will use a register field to help generate SID[7:0].
 *    Due to Display HW restrictions, that register field value cannot exceed 191
 *    for the NISO SIDs that are generated. ISO SIDs do not have this limitation
 *    and can be upto 255.
 * 3. Due to above 2 resrictions we'll limit Display SID[7:0] values 0 to 126.
 *    - Display ISO SIDs: From (TEGRA_SID_DISP) upto (TEGRA_SID_DISP + 63U)
 *    - Display NISO SIDs: From (TEGRA_SID_DISP + 64U) upto (TEGRA_SID_DISP + 126U)
 */

#define TEGRA_SID_DISP_NISO         (TEGRA_SID_DISP + 64U)

#define TEGRA_SID_DISP_VDISP(x)        (TEGRA_SID_DISP + (x) + 1)
#define TEGRA_SID_DISP_NISO_VDISP(x)   (TEGRA_SID_DISP_NISO + (x) + 1)
