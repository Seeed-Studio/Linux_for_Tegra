...
const struct tee_file_operations *rpmb_ops;
...
static TEE_Result open_dirh(struct tee_fs_dirfile_dirh **dirh)
{
	...
	TEE_Result (*rpmb_fs_raw_open)(const char *, bool,
				       struct tee_file_handle **);

#ifdef CFG_RPMB_FS
	rpmb_fs_raw_open = tee_rpmb_fs_raw_open;
	rpmb_ops = &rpmb_fs_ops;
#elif defined CFG_NVME_RPMB_FS
	rpmb_fs_raw_open = tee_nvme_rpmb_fs_raw_open;
	rpmb_ops = &nvme_rpmb_fs_ops;
#else
	assert(0);
#endif

	res = (*rpmb_fs_raw_open)(fname, false, &ree_fs_rpmb_fh);
	if (!res) {
		size_t l = sizeof(hash);

		res = rpmb_ops->read(ree_fs_rpmb_fh, 0, hash, &l);
		if (res)
			return res;
		if (l == sizeof(hash))
			hashp = hash;
	} else if (res == TEE_ERROR_ITEM_NOT_FOUND) {
		res = (*rpmb_fs_raw_open)(fname, true, &ree_fs_rpmb_fh);
	}
	if (res)
		return res;
	...
	if (res == TEE_ERROR_ITEM_NOT_FOUND) {
		if (hashp) {
			if (IS_ENABLED(CFG_REE_FS_ALLOW_RESET)) {
				...
				res = rpmb_ops->truncate(ree_fs_rpmb_fh, 0);
				if (res) {
...
static TEE_Result commit_dirh_writes(struct tee_fs_dirfile_dirh *dirh)
{
	...
	if (res)
		return res;
	return rpmb_ops->write(ree_fs_rpmb_fh, 0, hash, sizeof(hash));
}
