...
#endif
#ifdef CFG_NVME_RPMB_FS
extern const struct tee_file_operations nvme_rpmb_fs_ops;

TEE_Result tee_nvme_rpmb_fs_raw_open(const char *fname, bool create,
				     struct tee_file_handle **fh);

/**
 * Weak function which can be overridden by platforms to indicate that the NVME-RPMB
 * key is ready to be written. Defaults to true, platforms can return false to
 * prevent a RPMB key write in the wrong state.
 */
bool plat_nvme_rpmb_key_is_ready(void);
#endif
...
#endif
#ifdef CFG_NVME_RPMB_FS
	case TEE_STORAGE_PRIVATE_NVME_RPMB:
		return &nvme_rpmb_fs_ops;
#endif
	default:
		return NULL;
