...
static TEE_Result stmm_handle_variable_authentication (struct thread_scall_regs *regs)
{
	TEE_Result res = TEE_ERROR_GENERIC;
	uint32_t hash_len = SVC_REGS_A2 (regs);
	uint8_t *hash_val = (uint8_t *)SVC_REGS_A3 (regs);

	res =  jetson_user_key_pta_uefi_vars_auth(hash_val, hash_len);
	if (res) {
		EMSG ("Failed to get signed CMAC %x\n", res);
	}
...
static void spm_handle_direct_req(struct thread_scall_regs *regs)
{
	...
	} else if (dst_id == ffa_variable_authentication) {
		stmm_handle_variable_authentication (regs);
	}else {
