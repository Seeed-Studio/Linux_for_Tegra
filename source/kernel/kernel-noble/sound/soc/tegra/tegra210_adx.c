...
static void tegra210_adx_write_map_ram(struct tegra210_adx *adx)
{
	...
	for (i = 0; i < adx->soc_data->byte_mask_size; i++)
		regmap_write(adx->regmap, TEGRA210_ADX_IN_BYTE_EN0 +
			(i * TEGRA210_ADX_AUDIOCIF_CH_STRIDE), adx->byte_mask[i]);
}
				      ...
				      unsigned int reg)
{
	...
	cif_conf.has_max_32ch = (adx->soc_data->max_ch == 32) ? true : false;
...
static int tegra210_adx_platform_probe(struct platform_device *pdev)
{
	...
	/* Update the input dai max channel as per soc */
