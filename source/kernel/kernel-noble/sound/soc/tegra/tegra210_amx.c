...
static void tegra210_amx_write_map_ram(struct tegra210_amx *amx)
{
	...
	for (i = 0; i < amx->soc_data->byte_mask_size; i++)
		regmap_write(amx->regmap, TEGRA210_AMX_OUT_BYTE_EN0 +
			(i * TEGRA210_AMX_AUDIOCIF_CH_STRIDE), amx->byte_mask[i]);
}
				      ...
				      unsigned int reg)
{
	...
	cif_conf.has_max_32ch = (amx->soc_data->max_ch == 32) ? true : false;
...
static const struct snd_soc_dapm_route tegra210_amx_c2c_routes[] = {
	/* XBAR routes */
...
static int tegra210_amx_component_probe(struct snd_soc_component *component)
{
	...
	if (card->component_chaining) {
		route = tegra210_amx_routes;
		num_route = ARRAY_SIZE(tegra210_amx_routes);
	} else {
		route = tegra210_amx_c2c_routes;
		num_route = ARRAY_SIZE(tegra210_amx_c2c_routes);
	}
...
static int tegra210_amx_platform_probe(struct platform_device *pdev)
{
	...
	/* Update the output dai max channel as per soc */
